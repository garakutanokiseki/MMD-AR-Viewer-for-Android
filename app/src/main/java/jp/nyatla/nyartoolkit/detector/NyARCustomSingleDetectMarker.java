...
/**
 * このクラスは、1個のマーカを取り扱うマーカ検出器のベースクラスです。 アプリケーションからは、このクラスを継承した
 * {@link NyARSingleDetectMarker}を使います。
...
public abstract class NyARCustomSingleDetectMarker {
	...
	/**
	 * 内部関数です。 この関数は、thisの二次元矩形情報プロパティを更新します。
	 * 
	 * @param i_coord
	 * @param i_vertex_index
	 * @throws NyARException
	 */
	protected void updateSquareInfo(NyARIntCoordinates i_coord,
	...
	/**
	 * コンストラクタです。 実際の初期化は、継承クラスから、{@link #initInstance}関数をコールして実行します。
	 */
	 ...
	 * @throws NyARException
	 */
	protected void initInstance(INyARColorPatt i_patt_inst,
			NyARSquareContourDetector i_sqdetect_inst,
			INyARTransMat i_transmat_inst, INyARRasterFilter_Rgb2Bin i_filter,
			NyARParam i_ref_param, NyARCode i_ref_code, double i_marker_width)
			throws NyARException {
		...
		this._deviation_data = new NyARMatchPattDeviationColorData(
				i_ref_code.getWidth(), i_ref_code.getHeight());
		this._coordline = new NyARCoord2Linear(i_ref_param.getScreenSize(),
				i_ref_param.getDistortionFactor());
	 ...
	 * 関数の成功後は、マーカの姿勢行列と、一致度を、それぞれ{@link #getTransmationMatrix}と
	 * {@link #getConfidence}から得ることができます。
	 * 
	 * @param i_raster
	 *            マーカーを検出する画像。画像のサイズは、コンストラクタに指定した{@link NyARParam}
	 *            オブジェクトと一致していなければなりません。
	 * @return マーカーが検出できたかを、真偽値で返します。
	 * @throws NyARException
	 */
	protected boolean detectMarkerLite(INyARRgbRaster i_raster)
			throws NyARException {
		// サイズチェック
	...
	/**
	 * この関数は、検出したマーカーの変換行列を計算して、o_resultへ値を返します。 直前に実行した
	 * {@link #detectMarkerLite}が成功していないと使えません。
	 * 
	 * @param o_result
	 *            変換行列を受け取るオブジェクト。
	 * @throws NyARException
	...
	public void getTransmationMatrix(NyARTransMatResult o_result)
		...
		if (this._is_continue) {
			this._transmat.transMatContinue(this._square, this._offset,
					o_result, o_result);
		} else {
	...
	/**
	 * この関数は、マーカーの画像上の位置を格納する、{@link NyARSquare}への参照値を返します。 直前に実行した
	 * {@link #detectMarkerLite}が成功していないと使えません。 返却値の内容は、次に
	 * {@link #detectMarkerLite}を実行するまで有効です。
	 * 
	 * @return 矩形情報への参照値。
	 */
	...
	/**
	 * この関数は、検出したマーカーと登録済パターンとの、一致度を返します。 直前に実行した{@link #detectMarkerLite}
	 * が成功していないと使えません。 値は、0&lt;=n<1の間の数値を取ります。 一般的に、一致度が低い場合は、マーカを誤認識しています。
	 * 
	 * @return 一致度の数値。
	 */
	...
	/**
	 * デバック関数。 privateメンバにアクセスするためのトンネルです。
	 * 
	 * @return デバック用オブジェクトを格納した配列。
	 */
