...
/**
 * このクラスは、複数のマーカを取り扱うマーカ検出器です。 登録したn個のARマーカに対応するマーカを入力画像から検出し、その変換行列と一致度を返します。
...
public class NyARDetectMarker {
	...
	private class RleDetector extends NyARSquareContourDetector_Rle {
		// 公開プロパティ
		public NyARDetectMarkerResultStack result_stack = new NyARDetectMarkerResultStack(
				NyARDetectMarker.AR_SQUARE_MAX);
		// 参照インスタンス
		...
		public RleDetector(INyARColorPatt i_inst_patt, NyARCode[] i_ref_code,
				int i_num_of_code, NyARParam i_param) throws NyARException {
			...
			this._match_patt[0] = new NyARMatchPatt_Color_WITHOUT_PCA(
					i_ref_code[0]);
			for (int i = 1; i < i_num_of_code; i++) {
				// 解像度チェック
				if (cw != i_ref_code[i].getWidth()
						|| ch != i_ref_code[i].getHeight()) {
					throw new NyARException();
				}
				this._match_patt[i] = new NyARMatchPatt_Color_WITHOUT_PCA(
						i_ref_code[i]);
			}
			this._inst_patt = i_inst_patt;
			this._coordline = new NyARCoord2Linear(i_param.getScreenSize(),
					i_param.getDistortionFactor());
		...
		/**
		 * 矩形が見付かるたびに呼び出されます。 発見した矩形のパターンを検査して、方位を考慮した頂点データを確保します。
		 */
		protected void onSquareDetect(NyARIntCoordinates i_coord,
				int[] i_vertex_index) throws NyARException {
			...
			for (int i = 0; i < 4; i++) {
				...
				this._coordline.coord2Line(i_vertex_index[idx],
						i_vertex_index[(idx + 1) % 4], i_coord, sq.line[i]);
			}
			for (int i = 0; i < 4; i++) {
				// 直線同士の交点計算
				if (!sq.line[i].crossPos(sq.line[(i + 3) % 4], sq.sqvertex[i])) {
					throw new NyARException();
					// ここのエラー復帰するならダブルバッファにすればOK
				}
			}
		}
	...
	/**
	 * コンストラクタです。 同一でない複数のマーカを検出する検出器を作成します。
	 * 
	 * @param i_param
	 *            カメラパラメータを指定します。このサイズは、{@link #detectMarkerLite}
	 *            に入力する画像と同じである必要があります。
	 * @param i_code
	 *            検出するマーカーパターンを格納した、{@link NyARCode}
	 *            の配列を指定します。配列には、先頭から、0から始まるID番号が割り当てられます。 このIDは、
	 *            {@link #getARCodeIndex}で取得できるID値であり、マーカパターンの識別に使います。 配列要素の
	 *            {@link NyARCode}は、全て同じ解像度にしてください。
	 * @param i_marker_width
	 *            正方形マーカの物理サイズをmm単位で指定します。
	 * @param i_number_of_code
	 *            i_codeの有効な個数を指定します。
	 * @param i_input_raster_type
	 *            {@link #detectMarkerLite}に入力するラスタの画素形式を指定します。 この値は、
	 *            {@link INyARRgbRaster#getBufferType}関数の戻り値を利用します。
	 * @throws NyARException
	 */
	public NyARDetectMarker(NyARParam i_param, NyARCode[] i_code,
			double[] i_marker_width, int i_number_of_code,
	...
	/**
	 * この関数は、インスタンスを初期化します。 コンストラクタから呼び出します。
	 * 
	 * @see NyARDetectMarker#NyARDetectMarker(NyARParam, NyARCode[], double[],
	 *      int, int)
	 * @param i_ref_param
	 *            Check see also
	 * @param i_ref_code
	 ...
	 * @throws NyARException
	 */
	protected void initInstance(NyARParam i_ref_param, NyARCode[] i_ref_code,
			double[] i_marker_width, int i_number_of_code,
			int i_input_raster_type) throws NyARException {
		...
		// NyARToolkitプロファイル
		this._square_detect = new RleDetector(new NyARColorPatt_Perspective_O2(
				cw, ch, 4, 25, i_input_raster_type), i_ref_code,
				i_number_of_code, i_ref_param);
	 ...
	 * @return 検出したマーカーの数を返します。 マーカーが見つからない場合は0を返します。
	 * @throws NyARException
	 */
	public int detectMarkerLite(INyARRgbRaster i_raster, int i_threshold)
		...
		// ラスタを２値イメージに変換する.
		((NyARRasterFilter_ARToolkitThreshold) this._tobin_filter)
				.setThreshold(i_threshold);
	...
	/**
	 * この関数は、i_index番目に検出したマーカの、変換行列を計算します。 直前に実行した{@link #detectMarkerLite}
	 * が成功していないと使えません。
	 * 
	 * @param i_index
	 *            検出結果のインデックス番号を指定します。 この値は、0から{@link #detectMarkerLite}
	 *            関数の戻り値-1の数です。
	 * @param o_result
	 *            結果値を受け取るオブジェクト
	 * @throws NyARException
	...
	public void getTransmationMatrix(int i_index, NyARTransMatResult o_result)
			throws NyARException {
		final NyARDetectMarkerResult result = this._square_detect.result_stack
				.getItem(i_index);
		// 一番一致したマーカーの位置とかその辺を計算
		if (_is_continue) {
			_transmat.transMatContinue(result.square,
					this._offset[result.arcode_id], o_result, o_result);
		} else {
			_transmat.transMat(result.square, this._offset[result.arcode_id],
					o_result);
		}
		return;
	}
	...
	/**
	 * この関数は、i_index番目に検出したマーカの、一致度を返します。 直前に実行した{@link #detectMarkerLite}
	 * が成功していないと使えません。
	 * 
	 * @param i_index
	 *            検出結果のインデックス番号を指定します。 この値は、0から{@link #detectMarkerLite}
	...
	/**
	 * この関数は、i_index番目に検出したマーカの、ID番号を返します。 直前に実行した{@link #detectMarkerLite}
	 * が成功していないと使えません。
	 * 
	 * @param i_index
	 *            検出結果のインデックス番号を指定します。 この値は、0から{@link #detectMarkerLite}
	 *            関数の戻り値-1の数です。
	 * @return ID番号です。この値は、コンストラクタでマーカパターンを登録したときに決まる、シリアル番号です。
	 * @throws NyARException
	 */
...
/** 内部クラスです。ユーザが使用することはありません */
class NyARDetectMarkerResultStack extends
		NyARObjectStack<NyARDetectMarkerResult> {
