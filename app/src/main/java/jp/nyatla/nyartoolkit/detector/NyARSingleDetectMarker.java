...
/**
 * このクラスは、1個のマーカを取り扱うマーカ検出器です。 登録した１個のARマーカに対応するマーカを入力画像から検出し、その変換行列と一致度を返します。
 * <p>
 * 簡単な使い方
 * <ol>
 * <li>インスタンスを作成します。パラメータには、計算アルゴリズムと入力画像形式、カメラパラメータ、検出するマーカがあります。
 * <li>{@link #detectMarkerLite}関数に画像と敷居値を入力して、マーカを検出します。
 * <li>マーカが見つかると、インスタンスのプロパティが更新されます。{@link #getConfidence}
...
public class NyARSingleDetectMarker extends NyARCustomSingleDetectMarker {
	...
	/**
	 * ARToolKit互換アルゴリズムと、NyARToolKitのアルゴリズムの混合です。2D系にNyARToolkit,3D
	 * 系にARToolKitのアルゴリズムを選択します。
	 */
	...
	private class RleDetector extends NyARSquareContourDetector_Rle {
		...
		public RleDetector(NyARCustomSingleDetectMarker i_parent,
	...
	class ARTKDetector extends NyARSquareContourDetector_ARToolKit {
		...
		public ARTKDetector(NyARCustomSingleDetectMarker i_parent,
				NyARIntSize i_size) throws NyARException {
			super(i_size);
			this._parent = i_parent;
		}
	...
	/**
	 * コンストラクタです。 指定した1種のマーカを1個検出するインスタンスを作ります。
	 * 
	 * @param i_param
	 ...
	 */
	public NyARSingleDetectMarker(NyARParam i_param, NyARCode i_code,
			double i_marker_width, int i_input_raster_type, int i_profile_id)
			throws NyARException {
		super();
		initialize(i_param, i_code, i_marker_width, i_input_raster_type,
				i_profile_id);
		return;
	}

	/**
	 * コンストラクタです。 指定した1種のマーカを1個検出するインスタンスを作ります。 i_profile_idに
	 * {@link #PF_NYARTOOLKIT}を選択した
	 * {@link #NyARSingleDetectMarker(NyARParam, NyARCode, double, int, int)}
	 * と同じです。
	 * 
	 ...
	 */
	public NyARSingleDetectMarker(NyARParam i_param, NyARCode i_code,
			double i_marker_width, int i_input_raster_type)
			throws NyARException {
		super();
		initialize(i_param, i_code, i_marker_width, i_input_raster_type,
				PF_NYARTOOLKIT);
	...
	/**
	 * この関数は、インスタンスを初期化します。 引数は、{@link #NyARSingleDetectMarker}の対応する引数と同じです。
	 * 
	 * @see #NyARSingleDetectMarker(NyARParam, NyARCode, double, int, int)
	 * @param i_ref_param
	 *            Check see also
	 ...
	 * @throws NyARException
	 */
	private void initialize(NyARParam i_ref_param, NyARCode i_ref_code,
			double i_marker_width, int i_input_raster_type, int i_profile_id)
			throws NyARException {
		final NyARRasterFilter_ARToolkitThreshold th = new NyARRasterFilter_ARToolkitThreshold(
				100, i_input_raster_type);
		...
		case PF_ARTOOLKIT_COMPATIBLE:
			patt_inst = new NyARColorPatt_O3(i_ref_code.getWidth(),
					i_ref_code.getHeight());
		...
		case PF_NYARTOOLKIT_ARTOOLKIT_FITTING:
			patt_inst = new NyARColorPatt_Perspective_O2(i_ref_code.getWidth(),
					i_ref_code.getHeight(), 4, 25, i_input_raster_type);
			sqdetect_inst = new RleDetector(this, i_ref_param.getScreenSize());
			transmat_inst = new NyARTransMat_ARToolKit(i_ref_param);
			break;
		case PF_NYARTOOLKIT:// default
			// patt_inst=new NyARColorPatt_Perspective(i_ref_code.getWidth(),
			// i_ref_code.getHeight(),4,25);
			patt_inst = new NyARColorPatt_Perspective_O2(i_ref_code.getWidth(),
					i_ref_code.getHeight(), 4, 25, i_input_raster_type);
			sqdetect_inst = new RleDetector(this, i_ref_param.getScreenSize());
		...
		super.initInstance(patt_inst, sqdetect_inst, transmat_inst, th,
				i_ref_param, i_ref_code, i_marker_width);
