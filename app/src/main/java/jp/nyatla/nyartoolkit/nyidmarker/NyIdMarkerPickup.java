...
/**
 * このクラスは、ラスタ画像に定義したの任意矩形から、NyIdパターンのデータを読み取ります。 読み取り結果は、
 * {@link NyIdMarkerPattern}クラスのオブジェクトに出力します。
 * 
 */
public class NyIdMarkerPickup {
	 ...
	 * @return 抽出に成功するとtrue.失敗するとfalseです。
	 * @throws NyARException
	 */
	public final boolean pickFromRaster(INyARRgbRaster image,
			NyARDoublePoint2d[] i_vertex, NyIdMarkerPattern o_data,
			NyIdMarkerParam o_param) throws NyARException {
		// 遠近法のパラメータを計算
	 ...
	 */
	public final boolean pickFromRaster(INyARRgbRaster image,
			NyARIntPoint2d[] i_vertex, NyIdMarkerPattern o_data,
			NyIdMarkerParam o_param) throws NyARException {
		if (!this._perspective_reader.setSourceSquare(i_vertex)) {
			return false;
		}
	...
	/**
	 * i_imageから、idマーカを読みだします。 o_dataにはマーカデータ、o_paramにはマーカのパラメータを返却します。
	 * 
	 * @param image
	 * @param i_vertex
	 * @param o_data
	 ...
	 * @throws NyARException
	 */
	private final boolean _pickFromRaster(INyARRgbRaster image,
			NyIdMarkerPattern o_data, NyIdMarkerParam o_param)
			throws NyARException {
		...
		if (!this._perspective_reader.readDataBits(reader, raster_size, th,
				encoder)) {
			return false;
		}
...
/**
 * {@link NyIdMarkerPickup}がラスタからPerspective変換して読みだすためのクラス 画像処理全般を担当します。
 * ユーザがこのクラスを使うことはありません。
 * 
 */
final class PerspectivePixelReader {
	private static int READ_RESOLUTION = 100;
	private NyARPerspectiveParamGenerator _param_gen = new NyARPerspectiveParamGenerator_O1(
			1, 1);
	...
	public boolean setSourceSquare(NyARIntPoint2d[] i_vertex)
			throws NyARException {
		return this._param_gen.getParam(READ_RESOLUTION, READ_RESOLUTION,
				i_vertex, this._cparam);
	}
	 ...
	 * @throws NyARException
	 */
	private boolean rectPixels(INyARRgbPixelReader i_reader,
			NyARIntSize i_raster_size, int i_lt_x, int i_lt_y, int i_step_x,
			int i_step_y, int i_width, int i_height, int i_out_st, int[] o_pixel)
			throws NyARException {
		...
		for (int i = 0; i < i_height; i++) {
			...
			for (int i2 = 0; i2 < i_width; i2++) {
				int index = i2 * 3;
				o_pixel[out_index] = (pixcel_temp[index + 0]
						+ pixcel_temp[index + 1] + pixcel_temp[index + 2]) / 3;
				out_index++;
			}
		}
		return true;
	...
	/**
	 * パターン分析関数。 i_freqにあるゼロクロス点の周期が、等間隔か調べます。
	...
	/**
	 * パターン分析関数。 i_freq_count_tableとi_freq_tableの内容を調査し、最も大きな周波数成分を返します。
	 * 
	 * @param i_freq_count_table
	 * @param i_freq_table
	 * @param o_freq_table
	 * @return 見つかれば0以上、密辛ければ0未満
	 */
	private static int getMaxFreq(int[] i_freq_count_table, int[] i_freq_table,
			int[] o_freq_table) {
		// 一番成分の大きいものを得る
		int index = -1;
		int max = 0;
	...
	private final int[] _freq_table = new int[(MAX_FREQ * 2 - 1) * MAX_FREQ * 2
			/ 2];
	 ...
	 *            暗点の敷居値
	 * @param o_edge_index
	 *            検出したエッジ位置(H->L,L->H)のインデクスを受け取る配列。 [FRQ_POINTS]以上の配列を指定すること。
	 * @return 周波数の値。失敗すると-1
	 * @throws NyARException
	 */
	public int getRowFrequency(INyARRgbPixelReader i_reader,
			NyARIntSize i_raster_size, int i_y1, int i_th_h, int i_th_l,
			int[] o_edge_index) throws NyARException {
		// 3,4,5,6,7,8,9,10
		...
		for (int i = 0; i < FREQ_SAMPLE_NUM; i++) {
			...
			final int freq_t = getFreqInfo(pixcel_temp, i_th_h, i_th_l,
					o_edge_index);
	 ...
	 * @throws NyARException
	 */
	public int getColFrequency(INyARRgbPixelReader i_reader,
			NyARIntSize i_raster_size, int i_x1, int i_th_h, int i_th_l,
			int[] o_edge_index) throws NyARException {
		final double[] cpara = this._cparam;
		final int[] ref_x = this._ref_x;
		final int[] ref_y = this._ref_y;
	 ...
	 * @return
	 */
	private static int getFreqInfo(int[] i_pixcels, int i_th_h, int i_th_l,
			int[] o_edge_index) {
		// トークンを解析して、周波数を計算
		int i = 0;
		int frq_l2h = 0;
	...
	private static final int THRESHOLD_SAMPLE = THRESHOLD_PIXEL
			* THRESHOLD_PIXEL;
	private static final int THRESHOLD_SAMPLE_LT = THRESHOLD_EDGE;
	private static final int THRESHOLD_SAMPLE_RB = 100 - THRESHOLD_WIDTH
			- THRESHOLD_EDGE;
	 ...
	 * @throws NyARException
	 */
	public void detectThresholdValue(INyARRgbPixelReader i_reader,
			NyARIntSize i_raster_size, TThreshold o_threshold)
			throws NyARException {
		final int[] th_pixels = this._th_pixels;

		// 左上のピックアップ領域からピクセルを得る(00-24)
		rectPixels(i_reader, i_raster_size, THRESHOLD_SAMPLE_LT,
				THRESHOLD_SAMPLE_LT, THRESHOLD_STEP, THRESHOLD_STEP,
				THRESHOLD_PIXEL, THRESHOLD_PIXEL, 0, th_pixels);

		// 左下のピックアップ領域からピクセルを得る(25-49)
		rectPixels(i_reader, i_raster_size, THRESHOLD_SAMPLE_LT,
				THRESHOLD_SAMPLE_RB, THRESHOLD_STEP, THRESHOLD_STEP,
				THRESHOLD_PIXEL, THRESHOLD_PIXEL, THRESHOLD_SAMPLE, th_pixels);

		// 右上のピックアップ領域からピクセルを得る(50-74)
		rectPixels(i_reader, i_raster_size, THRESHOLD_SAMPLE_RB,
				THRESHOLD_SAMPLE_LT, THRESHOLD_STEP, THRESHOLD_STEP,
				THRESHOLD_PIXEL, THRESHOLD_PIXEL, THRESHOLD_SAMPLE * 2,
				th_pixels);

		// 右下のピックアップ領域からピクセルを得る(75-99)
		rectPixels(i_reader, i_raster_size, THRESHOLD_SAMPLE_RB,
				THRESHOLD_SAMPLE_RB, THRESHOLD_STEP, THRESHOLD_STEP,
				THRESHOLD_PIXEL, THRESHOLD_PIXEL, THRESHOLD_SAMPLE * 3,
				th_pixels);
		...
		// LT
		if (getHighPixelCenter(0, th_pixels, THRESHOLD_PIXEL, THRESHOLD_PIXEL,
				th, tpt)) {
		...
		// LB
		if (getHighPixelCenter(THRESHOLD_SAMPLE * 1, th_pixels,
				THRESHOLD_PIXEL, THRESHOLD_PIXEL, th, tpt)) {
		...
		// RT
		if (getHighPixelCenter(THRESHOLD_SAMPLE * 2, th_pixels,
				THRESHOLD_PIXEL, THRESHOLD_PIXEL, th, tpt)) {
		...
		// RB
		if (getHighPixelCenter(THRESHOLD_SAMPLE * 3, th_pixels,
				THRESHOLD_PIXEL, THRESHOLD_PIXEL, th, tpt)) {
	...
	private boolean getHighPixelCenter(int i_st, int[] i_pixels, int i_width,
			int i_height, int i_th, NyARIntPoint2d o_point) {
		int rp = i_st;
		int pos_x = 0;
		int pos_y = 0;
		int number_of_pos = 0;
	...
	private int detectDataBitsIndex(INyARRgbPixelReader i_reader,
			NyARIntSize i_raster_size, PerspectivePixelReader.TThreshold i_th,
			double[] o_index_row, double[] o_index_col) throws NyARException {
		// 周波数を測定
		final int[] freq_index1 = this.__detectDataBitsIndex_freq_index1;
		final int[] freq_index2 = this.__detectDataBitsIndex_freq_index2;

		int frq_t = getRowFrequency(i_reader, i_raster_size, i_th.lt_y,
				i_th.th_h, i_th.th_l, freq_index1);
		int frq_b = getRowFrequency(i_reader, i_raster_size, i_th.rb_y,
				i_th.th_h, i_th.th_l, freq_index2);
		// 周波数はまとも？
		...
		for (int i = 0; i < freq_h + freq_h - 1; i++) {
			o_index_row[i * 2] = ((index[i + 1] - index[i]) * 2 / 5 + index[i])
					+ FRQ_EDGE;
			o_index_row[i * 2 + 1] = ((index[i + 1] - index[i]) * 3 / 5 + index[i])
					+ FRQ_EDGE;
		}

		final int frq_l = getColFrequency(i_reader, i_raster_size, i_th.lt_x,
				i_th.th_h, i_th.th_l, freq_index1);
		final int frq_r = getColFrequency(i_reader, i_raster_size, i_th.rb_x,
				i_th.th_h, i_th.th_l, freq_index2);
		// 周波数はまとも？
	 ...
	 * @throws NyARException
	 */
	public boolean readDataBits(INyARRgbPixelReader i_reader,
			NyARIntSize i_raster_size, PerspectivePixelReader.TThreshold i_th,
			MarkerPattEncoder o_bitbuffer) throws NyARException {
		final int raster_width = i_raster_size.w;
		final int raster_height = i_raster_size.h;
		...
		// 読み出し位置を取得
		final int size = detectDataBitsIndex(i_reader, i_raster_size, i_th,
				index_x, index_y);
		...
		for (int i = 0; i < resolution; i++) {
			...
			for (int i2 = 0; i2 < resolution; i2++) {
				...
				if (xx < 0 || xx >= raster_width || yy < 0
						|| yy >= raster_height) {
					ref_x[pt] = xx < 0 ? 0
							: (xx >= raster_width ? raster_width - 1
									: raster_width);
					ref_y[pt] = yy < 0 ? 0
							: (yy >= raster_height ? raster_height - 1
									: raster_height);
				}
				pt++;
			...
			for (int i2 = 0; i2 < resolution; i2++) {
				int index = i2 * 3 * 4;
				int pixel = (pixcel_temp[index + 0] + pixcel_temp[index + 1]
						+ pixcel_temp[index + 2] + pixcel_temp[index + 3]
						+ pixcel_temp[index + 4] + pixcel_temp[index + 5]
						+ pixcel_temp[index + 6] + pixcel_temp[index + 7]
						+ pixcel_temp[index + 8] + pixcel_temp[index + 9]
						+ pixcel_temp[index + 10] + pixcel_temp[index + 11])
						/ (4 * 3);
				// 暗点を1、明点を0で表現します。
 ...
 * ピクセルセットから、マーカーデータへエンコードする処理を定義します。 暫定として、5x5と、7x7のマーカについて定義します。
 */
class MarkerPattEncoder {
	private static final int[] _bit_table_3 = { 25, 26, 27, 28, 29, 30, 31, 48,
			9, 10, 11, 12, 13, 32, 47, 24, 1, 2, 3, 14, 33, 46, 23, 8, 0, 4,
			15, 34, 45, 22, 7, 6, 5, 16, 35, 44, 21, 20, 19, 18, 17, 36, 43,
			42, 41, 40, 39, 38, 37 };
	private static final int[] _bit_table_2 = { 9, 10, 11, 12, 13, 24, 1, 2, 3,
			14, 23, 8, 0, 4, 15, 22, 7, 6, 5, 16, 21, 20, 19, 18, 17 };
	private static final int[][] _bit_tables = { _bit_table_2, _bit_table_3,
			null, null, null, null, null, };
	/**
	 * RECT(0):[0]=(0) RECT(1):[1]=(1-8) RECT(2):[2]=(9-16),[3]=(17-24)
	...
	public void setBitByBitIndex(int i_index_no, int i_value) {
		...
		if (bit_no == 0) {
			...
			int sidx = (bit_no - 1) % 8;
			this._bits[bidx] = (this._bits[bidx] & (~(0x01 << sidx)))
					| (i_value << sidx);
		}
		return;
	}
	 ...
	 * @param o_out
	 *            マーカデータを受け取るオブジェクト。
	 * @return 成功すればマーカの方位を返却します。失敗すると-1を返します。
	 */
	...
	/**
	 * この関数は、ビットパックi_packを指定ビットだけ左シフトします。 現在未使用です。
	 * 
	 * @param i_pack
	 *            操作する配列
	 * @param i_start
	...
	public void shiftLeft(int[] i_pack, int i_start, int i_length, int i_ls) {
		...
		for (int i = i_length - 1; i >= 1; i--) {
			work[i] = (i_pack[i + i_start] << mod_shift)
					| (0xff & (i_pack[i + i_start - 1] >> (8 - mod_shift)));
		}
		work[0] = (i_pack[i_start] << mod_shift)
				| (0xff & (i_pack[i_start + i_length - 1] >> (8 - mod_shift)));
		// バイトシフト
