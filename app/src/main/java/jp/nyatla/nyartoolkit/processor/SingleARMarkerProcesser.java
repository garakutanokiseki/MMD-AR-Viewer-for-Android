 ...
 * <p>
 * イベントの説明- このクラスには、３個の自己コールバック関数があります。{@link SingleARMarkerProcesser}
 * は、以下のタイミングでこれらを呼び出します。 ユーザは継承クラスでこれらの関数に実装を行い、イベント駆動のアプリケーションを作成できます。
 * <ul>
 * <li>　{@link #onEnterHandler} -
 * 登録したマーカが初めて見つかった時に呼び出されます。ここに、発見したマーカに対応した初期処理を書きます。
 * <li>　{@link #onLeaveHandler} - 検出中のマーカが消失した時に呼び出されます。ここに、マーカの終期処理を書きます。
 * <li>　{@link #onUpdateHandler}-
...
public abstract class SingleARMarkerProcesser {
	...
	private class DetectSquare extends NyARSquareContourDetector_Rle {
		...
		public void setNyARCodeTable(NyARCode[] i_ref_code,
				int i_code_resolution) {
			/* unmanagedで実装するときは、ここでリソース解放をすること。 */
			this._deviation_data = new NyARMatchPattDeviationColorData(
					i_code_resolution, i_code_resolution);
			this._inst_patt = new NyARColorPatt_Perspective_O2(
					i_code_resolution, i_code_resolution, 4, 25,
					this._raster_type);
	...
	/**
	 * デフォルトコンストラクタ。 クラスを継承するときは、このコンストラクタを呼び出した後に、{@link #initInstance}
	 * 関数でインスタンスの初期化処理を実装します。
	 */
	...
	/**
	 * この関数は、インスタンスを初期化します。 継承先のクラスから呼び出してください。
	 * 
	 * @param i_param
	 *            カメラパラメータオブジェクト。このサイズは、{@link #detectMarker}
	 *            に入力する画像と同じサイズである必要があります。
	 * @param i_raster_type
	 *            {@link #detectMarker}関数に入力する画像のフォーマット。 この値には、
	...
	protected void initInstance(NyARParam i_param, int i_raster_type)
		...
		this._threshold_detect = new NyARRasterThresholdAnalyzer_SlidePTile(15,
				i_raster_type, 4);
	...
	/*
	 * 自動・手動の設定が出来ないので、コメントアウト public void setThreshold(int i_threshold) {
	 * this._threshold = i_threshold; return; }
	 */

	/**
	 * この関数は、検出するマーカパターンテーブルの配列を指定します。 マーカパターンには、配列の先頭から、0から始まるID番号を割り当てられます。
	 * このIDは、{@link #onEnterHandler}イベントハンドラに通知されるID番号に対応し、マーカパターンの識別に使います。
	 * 
	 * @param i_ref_code_table
	 *            マーカパターンテーブルにセットする配列です。配列にあるマーカパターンの解像度は、
	 ...
	 *            </p>
	 */
	public void setARCodeTable(NyARCode[] i_ref_code_table,
			int i_code_resolution, double i_marker_width) {
		...
		this._detectmarker
				.setNyARCodeTable(i_ref_code_table, i_code_resolution);
		this._offset.setSquare(i_marker_width);
		return;
	}

	/**
	 * この関数は、インスタンスの状態をリセットします。 状態をリセットすると、もしマーカを認識している場合には、
	 * {@link #onLeaveHandler}イベントハンドラがコールされ、未認識状態になります。
	 * 
	 * @param i_is_force
	...
	public void detectMarker(INyARRgbRaster i_raster) throws NyARException {
		// サイズチェック
		assert (this._bin_raster.getSize().isEqualSize(i_raster.getSize().w,
				i_raster.getSize().h));
		...
		// 認識状態を更新
		final boolean is_id_found = this.updateStatus(
				this._detectmarker.square, this._detectmarker.code_index);
	...
	/**
	 * この関数は、マーカパターンの一致率の敷居値を設定します。 敷居値は、0.0&lt;n&lt;1.0の範囲で指定します。
	 * 
	 * @param i_new_cf
	 *            新しくマーカを発見するときの閾値です。
	 * @param i_exist_cf
	 *            継続してマーカを追跡するときの閾値です。 i_new_cfの6割程度の値を指定すると良いでしょう。
	 */
	 ...
	 */
	private boolean updateStatus(NyARSquare i_square, int i_code_index)
			throws NyARException {
	...
	/**
	 * 自己コールバック関数です。 継承したクラスで、マーカ発見時の処理を実装してください。
	 * 
	 * @param i_code
	 *            検出したマーカパターンのID番号です。ID番号については、{@link #setARCodeTable}
	 *            の説明を参照してください。
	 */
	protected abstract void onEnterHandler(int i_code);

	/**
	 * 自己コールバック関数です。 継承したクラスで、マーカ消失時の処理を実装してください。
	 */
	protected abstract void onLeaveHandler();

	/**
	 * 自己コールバック関数です。 継承したクラスで、マーカ更新時の処理を実装してください。 引数の値の有効期間は、関数が終了するまでです。
	 * 
	 * @param i_square
	 *            現在のマーカ検出位置です。
	 * @param result
	 *            現在の姿勢変換行列です。
	 */
	protected abstract void onUpdateHandler(NyARSquare i_square,
