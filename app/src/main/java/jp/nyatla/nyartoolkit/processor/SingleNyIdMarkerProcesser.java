 ...
 * <p>
 * 自己コールバック関数の説明- このクラスには、３個の自己コールバック関数があります。{@link SingleARMarkerProcesser}
...
public abstract class SingleNyIdMarkerProcesser {
	...
	private class RleDetector extends NyARSquareContourDetector_Rle {
		...
		protected void onSquareDetect(NyARIntCoordinates i_coord,
			...
			if (!this._id_pickup.pickFromRaster(this._ref_raster, vertex,
					patt_data, param)) {
				return;
			}
			// エンコード
	 ...
	 *            {@link #detectMarker}関数に入力する画像の画素形式。 この値には、
	 *            {@link INyARRgbRaster#getBufferType}関数の戻り値を利用します。
	 * @throws NyARException
	 */
	protected void initInstance(NyARParam i_param,
			INyIdMarkerDataEncoder i_encoder, double i_marker_width,
			int i_raster_format) throws NyARException {
		// 初期化済？
	...
	public void detectMarker(INyARRgbRaster i_raster) throws NyARException {
		// サイズチェック
		if (!this._bin_raster.getSize().isEqualSize(i_raster.getSize().w,
				i_raster.getSize().h)) {
		...
		this._square_detect.init(i_raster, this._is_active ? this._data_current
				: null);
		this._square_detect.detectMarker(this._bin_raster);

		// 認識状態を更新(マーカを発見したなら、current_dataを渡すかんじ)
		final boolean is_id_found = updateStatus(this._square_detect.square,
				this._square_detect.marker_data);
	 ...
	 */
	private boolean updateStatus(NyARSquare i_square,
			INyIdMarkerData i_marker_data) throws NyARException {
	 ...
	 * 
	 * @param i_code
	 *            検出したIDマーカの内容をエンコードしたデータです。 使用したエンコーダに合せて、キャストしてください。 (例えば、
	 *            {@link NyIdMarkerDataEncoder_RawBit}をエンコーダに用いた時は、
	 *            {@link NyIdMarkerDataEncoder_RawBit}にキャストできます。)
	 */
