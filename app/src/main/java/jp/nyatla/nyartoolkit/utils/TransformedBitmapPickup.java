...
/**
 * このクラスは、姿勢変換行列を使用してマーカの周辺領域からパターンを取得する機能を持つラスタです。 画像の画素形式は、
 * {@link NyARBufferType#INT1D_X8R8G8B8_32}形式のです。
...
class TransformedBitmapPickup extends NyARColorPatt_Perspective_O2 {
	...
	/**
	 * コンストラクタです。 射影変換パラメータ、ラスタサイズ、サンプリング解像度からインスタンスを生成します。
	 * 
	 * @param i_ref_cparam
	 *            射影変換パラメータの参照値
	 * @param i_width
	 ...
	 *            ラスタの高さ
	 * @param i_resolution
	 *            ピクセルあたりのサンプリング解像度。1なら出力1ピクセルにつき1ピクセル、2なら1ピクセルにつき4ピクセル(2x2)
	 *            をサンプリングします。 小さい画像では数値が大きいほど良い結果が得られますが、時間がかかります。
	 */
	public TransformedBitmapPickup(
			NyARPerspectiveProjectionMatrix i_ref_cparam, int i_width,
			int i_height, int i_resolution) {
		// ANYラスタで構築
	 ...
	 * マーカから離れた場所になるほど、また、マーカの鉛直方向から外れるほど誤差が大きくなります。 ---- This function gets
	 * bitmap from the area defined by RECT(i_l,i_t,i_r,i_b) above transform
	 * matrix i_base_mat.
	 * 
	 * @param i_src_imege
	 *            詠み出し元の画像を指定します。
	 ...
	 * @return 画像の取得に成功するとtrueを返します。
	 */
	public boolean pickupImage2d(INyARRgbRaster i_src_imege, double i_l,
			double i_t, double i_r, double i_b, NyARTransMatResult i_base_mat)
