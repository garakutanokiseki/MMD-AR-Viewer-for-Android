...
/**
 * このクラスは、ARToolKit由来の姿勢行列最適化処理を実装します。 構造も含めた手順の実装が目的の為、低速です。 高速化した
 * {@link NyARRotMatrixOptimize_O2}を使用してください。
 */
public class NyARRotMatrixOptimize_Base implements INyARRotMatrixOptimize {
	 ...
	 *            射影変換オブジェクトの参照値。
	 */
	public NyARRotMatrixOptimize_Base(
	...
	private void createRotationMap(NyARDoublePoint3d i_angle, double i_factor,
			NyARDoubleMatrix33[] i_rot_matrix) {
		...
		for (t1 = 0; t1 < 3; t1++) {
			...
			for (t2 = 0; t2 < 3; t2++) {
				...
				for (t3 = 0; t3 < 3; t3++) {
					...
					mat_ptr.m00 = CACACB * cosc + SASA * cosc + SACACB * sinc
							- SACA * sinc;
					mat_ptr.m01 = -CACACB * sinc - SASA * sinc + SACACB * cosc
							- SACA * cosc;
					mat_ptr.m02 = CASB;
					mat_ptr.m10 = SACACB * cosc - SACA * cosc + SASACB * sinc
							+ CACA * sinc;
					mat_ptr.m11 = -SACACB * sinc + SACA * sinc + SASACB * cosc
							+ CACA * cosc;
					mat_ptr.m12 = SASB;
	...
	private final void getNewMatrix(NyARDoubleMatrix33 i_rot,
			NyARDoublePoint3d i_trans, NyARDoubleMatrix34 o_combo) {
	...
	private final NyARDoubleMatrix33[] __modifyMatrix_next_rot_matrix = NyARDoubleMatrix33
			.createArray(27);

	/**
	 * この関数は、回転行列を最適化します。 ARToolKitのarGetRotに相当します。
	 */
	public double modifyMatrix(NyARRotMatrix_ARToolKit io_rot,
		...
		for (int j = 0; j < 10; j++) {
			...
			for (i2 = 0; i2 < 27; i2++) {
				...
				err = 0.0;
				for (i = 0; i < 4; i++) {
					hx = combo.m00 * i_vertex3d[i].x + combo.m01
							* i_vertex3d[i].y + combo.m02 * i_vertex3d[i].z
							+ combo.m03;
					hy = combo.m10 * i_vertex3d[i].x + combo.m11
							* i_vertex3d[i].y + combo.m12 * i_vertex3d[i].z
							+ combo.m13;
					h = combo.m20 * i_vertex3d[i].x + combo.m21
							* i_vertex3d[i].y + combo.m22 * i_vertex3d[i].z
							+ combo.m23;
