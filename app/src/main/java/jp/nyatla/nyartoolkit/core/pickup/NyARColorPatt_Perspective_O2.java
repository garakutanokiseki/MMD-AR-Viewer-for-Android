...
public class NyARColorPatt_Perspective_O2 implements INyARColorPatt {
	...
	private void initializeInstance(int i_width, int i_height,
			int i_point_per_pix, int i_input_raster_type) {
		...
		this._perspective_reader = new NyARPerspectiveRasterReader(
				i_input_raster_type);
		return;
	}

	/**
	 * コンストラクタです。 エッジサイズ0,入力ラスタタイプの制限無しでインスタンスを作成します。
	 ...
	 */
	public NyARColorPatt_Perspective_O2(int i_width, int i_height,
			int i_point_per_pix) {
		initializeInstance(i_width, i_height, i_point_per_pix,
				NyARBufferType.NULL_ALLZERO);
		this._edge.setValue(0, 0);
		return;
	}

	/**
	 * コンストラクタです。 エッジサイズ,入力ラスタタイプの制限を指定してインスタンスを作成します。
	 * 
	 * @param i_width
	 *            取得画像の解像度幅
	 * @param i_height
	 ...
	 *            入力ラスタのバッファタイプ。高速化が期待できるバッファタイプについては、
	 *            {@link NyARPerspectiveRasterReader#NyARPerspectiveRasterReader}
	 *            を参照してください。
	 */
	public NyARColorPatt_Perspective_O2(int i_width, int i_height,
			int i_point_per_pix, int i_edge_percentage, int i_input_raster_type) {
		initializeInstance(i_width, i_height, i_point_per_pix,
				i_input_raster_type);
	...
	public boolean pickFromRaster(INyARRgbRaster image,
		...
		// 遠近法のパラメータを計算
		return this._perspective_reader.read4Point(image, i_vertexs,
				this._edge.x, this._edge.y, this._resolution, this);
	}
