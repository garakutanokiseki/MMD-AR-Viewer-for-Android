...
/**
 * このクラスは、0=a*x+b*y+cのパラメータを格納します。 x,yの増加方向は、x=L→R,y=B→Tです。 y軸が反転しているので注意してください。
 * 
 */
public class NyARLinear {
	 ...
	 * @return 交点が求まればtrue
	 */
	public final boolean crossPos(double i_a, double i_b, double i_c,
	...
	/**
	 * この関数は、2点を結ぶ直線式を計算して、インスタンスに格納します。 式の係数値は、正規化されます。
	 * 
	 * @param i_point1
	 *            点１
	 * @param i_point2
	 *            点２
	 * @return 直線式が求まれば、true
	 */
	public final boolean makeLinearWithNormalize(NyARIntPoint2d i_point1,
			NyARIntPoint2d i_point2) {
		return makeLinearWithNormalize(i_point1.x, i_point1.y, i_point2.x,
				i_point2.y);
	}
	 ...
	 * @param i_point2
	 *            点２
	 * @return 直線式が求るとtrueを返します。
	 */
	public final boolean makeLinearWithNormalize(NyARDoublePoint2d i_point1,
			NyARDoublePoint2d i_point2) {
		return makeLinearWithNormalize(i_point1.x, i_point1.y, i_point2.x,
				i_point2.y);
	}
	 ...
	 * @return 直線式が求るとtrueを返します。
	 */
	public final boolean makeLinearWithNormalize(double x1, double y1,
			double x2, double y2) {
		double dx = y2 - y1;
		double dy = x1 - x2;
	 ...
	 * @param o_point
	 *            交点を返却するオブジェクト
	 * @return 交点が求まれば、trueを返します。
	 */
	public final boolean normalLineCrossPos(double i_x, double i_y,
			NyARLinear i_linear, NyARDoublePoint2d o_point) {
		// thisを法線に変換
		double la = this.b;
		double lb = -this.a;
	...
	// * 交点が無い場合、無限大を返します。
	// */
	// public final double sqDistWithLinear(NyARLinear i_linear, double
	// i_x,double i_y)
	// {
	// //thisを法線に変換
	// double la=this.b;
	 ...
	 * @return 端点が求まればtrue
	 */
	public final boolean makeSegmentLine(int i_width, int i_height,
			NyARIntPoint2d[] o_point) {
		int idx = 0;
		...
		if (this.crossPos(0, -1, i_height - 1, ptr) && ptr.x >= 0
				&& ptr.x < i_width) {
		...
		if (this.crossPos(-1, 0, i_width - 1, ptr) && ptr.y >= 0
				&& ptr.y < i_height) {
			// x=i_right-1の線
			idx++;
			if (idx == 2) {
	 ...
	 * @return 端点が求まればtrue
	 */
	public final boolean makeSegmentLine(int i_left, int i_top, int i_width,
			int i_height, NyARIntPoint2d[] o_point) {
		int bottom = i_top + i_height;
		int right = i_left + i_width;
		int idx = 0;
		NyARIntPoint2d ptr = o_point[0];
		if (this.crossPos(0, -1, i_top, ptr) && ptr.x >= i_left
				&& ptr.x < right) {
			// y=rect.yの線
			idx++;
			ptr = o_point[idx];
		}
		if (this.crossPos(0, -1, bottom - 1, ptr) && ptr.x >= i_left
				&& ptr.x < right) {
			// y=(rect.y+rect.h-1)の線
			idx++;
			if (idx == 2) {
		...
		if (this.crossPos(-1, 0, i_left, ptr) && ptr.y >= i_top
				&& ptr.y < bottom) {
			// x=i_leftの線
			idx++;
			if (idx == 2) {
		...
		if (this.crossPos(-1, 0, right - 1, ptr) && ptr.y >= i_top
				&& ptr.y < bottom) {
			// x=i_right-1の線
			idx++;
			if (idx == 2) {
	 ...
	 */
	public boolean leastSquares(NyARDoublePoint2d[] i_points,
