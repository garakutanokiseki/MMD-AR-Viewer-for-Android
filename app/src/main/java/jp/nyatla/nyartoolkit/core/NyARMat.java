...
/**
 * このクラスは、可変サイズの行列計算機能を提供します。 ARToolKitのARMat構造体と、その処理関数に相当する機能があります。
 * 2x2,3x3,4x4行列については、NyAToolKitの行列計算クラス({@link NyARDoubleMatrix44},
 * {@link NyARDoubleMatrix33},{@link NyARDoubleMatrix22},) がありますので、こちらを使ってください。
 * <p>
...
public class NyARMat {
	...
	/**
	 * コンストラクタです。 行列のサイズを指定して、NyARMatオブジェクトを作成します。
	 * 
	 * @param i_row
	 *            行数です。
	 * @param i_clm
	...
	/**
	 * 行列のサイズを変更します。 関数を実行すると、行列の各値は不定になります。
	 * 
	 * @param i_row
	 *            新しい行数
	 * @param i_clm
	...
	/**
	 * 行列同士の掛け算を実行します。 i_mat_aとi_mat_bの積を計算して、thisへ格納します。
	 * 
	 * @param i_mat_a
	 *            計算元の行列A
	 * @param i_mat_b
	...
	public void matrixMul(NyARMat i_mat_a, NyARMat i_mat_b)
			throws NyARException {
		assert i_mat_a.clm == i_mat_b.row && this.row == i_mat_a.row
				&& this.clm == i_mat_b.clm;
	 ...
	 * 逆行列を計算して、thisへ格納します。
	 * 
	 * @return 　逆行列が計算できたかの、真偽値を返します。trueなら、逆行列が存在します。falseなら、逆行列はありません。
		...
		for (int n = 0; n < dimen; n++) {
			...
			for (int i = 0; i < dimen; i++) {
				if (i != n) {
					...
					work = ap_i[0];
					for (j = 0; j < dimen_1; j++) {// for(j = 1, wbp = wcp,work
													// = *wap;j < dimen ;j++,
													// wap++, wbp++)
	 ...
	 * 
	 * @param i_copy_from
	 *            コピー元の行列です。 この行列のサイズは、thisと同じでなければなりません。
	 */
	...
	/**
	 * 行列のバッファを返します。 返却値の有効期間に注意してください。 この値の有効時間は、次にこのこのインスタンスの関数を実行するまでの間です。
	 * 
	 * @return 行列のバッファ
	 */
	...
	/**
	 * ARToolKitのEX関数と同等な関数です。 詳細は不明です。
	 * 
	 * @param mean
	 *            不明
	 * @throws NyARException
	...
	/**
	 * ARToolKitのCENTER関数と同等な関数です。 詳細は不明です。
	 * 
	 * @param inout
	 *            不明
	 * @param mean
	...
	/**
	 * ARToolKitのx_by_xtと同等な関数です。 詳細は不明です。 この関数は未チェックの為、使用すると例外が発生します。
	 * 
	 * @param input
	 *            不明
	 * @param output
		...
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < row; j++) {
				if (j < i) {
					...
					NyARException.trap("未チェックのパス");
					in1 = input._m[i];// input.getRowArray(i);//in1 =
										// &(input->m[clm*i]);
					in2 = input._m[j];// input.getRowArray(j);//in2 =
										// &(input->m[clm*j]);
	...
	/**
	 * ARToolKitのxt_by_x関数と同等な関数です。 詳細は不明です。
	 * 
	 * @param input
	 *            不明
	 * @param i_output
	...
	/**
	 * ARToolKitのQRM関数と同等な関数です。 詳細は不明です。
	 * 
	 * @param dv
	 *            不明
	 * @throws NyARException
	...
	private void PCA_QRM(NyARVec dv) throws NyARException {
		...
		for (int h = dim - 1; h > 0; h--) {
			int j = h;
			while (j > 0
					&& Math.abs(ev_array[j]) > PCA_EPS
							* (Math.abs(dv_array[j - 1]) + Math
									.abs(dv_array[j]))) {// while(j>0 &&
															// fabs(ev->v[j])
															// >EPS*(fabs(dv->v[j-1])+fabs(dv->v[j])))
				// j--;
				j--;
			}
			if (j == h) {
			...
			do {
				...
				w = (dv_array[h - 1] - dv_array[h]) / 2;// w = (dv->v[h-1]
														// -dv->v[h]) / 2;//ここ？
				...
				x = dv_array[j] - dv_array[h] + t / (w + s);// x = dv->v[j]
															// -dv->v[h]
															// +t/(w+s);
				...
				for (int k = j; k < h; k++) {
					...
					w = dv_array[k] - dv_array[k + 1];// w = dv->v[k]
														// -dv->v[k+1];
					t = (w * s + 2 * c * ev_array[k + 1]) * s;// t = (w * s +2 *
																// c
																// *ev->v[k+1])
																// *s;
					...
					if (k > j) {
						...
						{
							ev_array[k] = c * ev_array[k] - s * y;// ev->v[k]= c
																	// *ev->v[k]-
																	// s * y;
						}
					}
					...
					for (int i = 0; i < dim; i++) {
						...
						L_m[k][i] = c * x - s * y;// a->m[k*dim+i] = c * x - s*
													// y;
						L_m[k + 1][i] = s * x + c * y;// a->m[(k+1)*dim+i] = s*
														// x + c * y;
					}
					if (k < h - 1) {
						NyARException.trap("未チェックパス");
	...
	/**
	 * ARToolKitのEV_create関数と同等な関数です。 詳細は不明です。
	 * 
	 * @param input
	 *            不明。
	 * @param u
	...
	/**
	 * ARToolKitのPCA関数と同等な関数です。 詳細は不明です。
	 * 
	 * @param output
	 *            不明
	 * @param o_ev
	...
	/**
	 * 現在の行列に主成分分析を実行して、結果をthisと引数へ格納します。 詳細は不明です。
	 * 
	 * @param o_evec
	 *            不明
	 * @param o_ev
	...
	/**
	 * ARToolKitの、arVecTridiagonalize関数と同等な関数です。 詳細は不明です。
	 * 
	 * @param d
	 *            不明
	 * @param e
	...
	private void vecTridiagonalize(NyARVec d, NyARVec e, int i_e_start)
		...
		for (int k = 0; k < dim - 2; k++) {
			...
			// wv1.v = &(v[k+1]);
			NyARException.trap("未チェックパス");
			e_vec[k + i_e_start] = vec.vecHousehold(k + 1);// e.v[k+i_e_start]=vec.vecHousehold(k+1);//e->v[k]=
															// arVecHousehold(&wv1);
			if (e_vec[k + i_e_start] == 0.0) {// if(e.v[k+i_e_start]==
												// 0.0){//if(e.v[k+i_e_start]==
												// 0.0){
				continue;
			}
			...
			for (int i = k + 1; i < dim; i++) {
				...
				for (int j = k + 1; j < i; j++) {
					NyARException.trap("未チェックのパス");
					s += this._m[j][i] * a_vec_k[j];// s += a_array[j][i]
													// *vec.v[j];//s
													// +=a.get(j*dim+i)
													// *v.get(j);//s
													// +=a->m[j*dim+i] * v[j];
				...
				for (int j = i; j < dim; j++) {
					NyARException.trap("未チェックのパス");
					s += this._m[i][j] * a_vec_k[j];// s += a_array[i][j]
													// *vec.v[j];//s
													// +=a.get(i*dim+j)
													// *v.get(j);//s
													// +=a->m[i*dim+j] * v[j];
			...
			for (int i = dim - 1; i > k; i--) {
				...
				for (int j = i; j < dim; j++) {
					NyARException.trap("未チェックパス");
					this._m[i][j] -= p * (d_vec[j] + q * a_vec_k[j]);// a.m[i][j]-=p*(d.v[j]
																		// +q*vec.v[j]);//a->m[i*dim+j]
																		// -=p*(d->v[j])
																		// +
																		// q*v[j];
				}
		...
		if (dim >= 2) {
			...
			e_vec[dim - 2 + i_e_start] = this._m[dim - 2][dim - 1];// e.v[dim-2+i_e_start]=a.m[dim-2][dim-1];//e->v[dim-2]
																	// =
																	// a->m[(dim-2)*dim+(dim-1)];
		}

		if (dim >= 1) {
			d_vec[dim - 1] = this._m[dim - 1][dim - 1];// d.v[dim-1]=a_array[dim-1][dim-1];//d->v[dim-1]
														// =a->m[(dim-1)*dim+(dim-1)];
		}
		NyARVec vec2 = this.wk_vecTridiagonalize_vec2;
		for (int k = dim - 1; k >= 0; k--) {
			a_vec_k = this._m[k];
			vec.setNewArray(a_vec_k, clm);// vec=this.getRowVec(k);//v
											// =a.getPointer(k*dim);//v =
											// &(a->m[k*dim]);
			if (k < dim - 2) {
				for (int i = k + 1; i < dim; i++) {
					// wv1.clm = wv2.clm = dim-k-1;
					...
					for (int j = k + 1; j < dim; j++) {
						NyARException.trap("未チェックパス");
						this._m[i][j] -= t * a_vec_k[j];// a_array[i][j]-=t*vec.v[j];//a.subValue(i*dim+j,t*v.get(j));//a->m[i*dim+j]-=
														// t * v[j];
					}
				}
			}
