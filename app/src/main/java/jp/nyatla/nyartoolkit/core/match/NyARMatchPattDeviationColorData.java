 ...
 * <p>
 * 使い方 - {@link #setRaster}関数で、差分画像を作成し、プロパティ取得関数でその情報を得ます。
 * </p>
 */
public class NyARMatchPattDeviationColorData {
	...
	/**
	 * この関数は、画素データを格納した配列を返します。 {@link NyARMatchPatt_Color_WITHOUT_PCA#evaluate}
	 * 関数等から使います。 R,G,Bの順番で、直列にデータを格納します。
	 */
	public int[] refData() {
		return this._data;
	}
	...
	/**
	 * この関数は、差分画像の強度値を返します。 強度値は、差分画像の画素を二乗した値の合計です。
	 * 
	 * @return 0&lt;nの強度値。
	 */
	...
	/**
	 * コンストラクタです。 差分画像のサイズを指定して、インスタンスを生成します。
	 * 
	 * @param i_width
	 *            差分画像のサイズ
	 * @param i_height
	 ...
	 *            形式のバッファを持つラスタの場合、他の形式よりも 何倍か高速に動作します。
	 */
	public void setRaster(INyARRgbRaster i_raster) throws NyARException {
		...
		case NyARBufferType.INT1D_X8R8G8B8_32:
			this._pow = setRaster_INT1D_X8R8G8B8_32(
					(int[]) i_raster.getBuffer(), this._size.w * this._size.h,
					this._optimize_for_mod, this._data);
			break;
		default:
			this._pow = setRaster_ANY(i_raster.getRgbPixelReader(), this._size,
					this._size.w * this._size.h, this._data);
			break;
		}
		return;
	}
	 ...
	 * @return
	 */
	private static final double setRaster_INT1D_X8R8G8B8_32(int[] i_buf,
			int i_number_of_pix, int i_for_mod, int[] o_out) {
	 ...
	 * @throws NyARException
	 */
	private static final double setRaster_ANY(INyARRgbPixelReader i_reader,
			NyARIntSize i_size, int i_number_of_pix, int[] o_out)
			throws NyARException {
		int width = i_size.w;
		int[] rgb = new int[3];
		int ave;// <PV/>
