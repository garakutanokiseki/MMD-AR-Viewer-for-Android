 ...
 * <p>
 * ラベル画像- ラべリング画像は、ラベルのインデクス値を格納します。インデクス値は、{@link #getIndexArray}で
 * 得られるインデクス配列を使って、ラベル番号に変換できます。 例えば、インデクス番号nの画素は、ラベル番号L[n]になります。
 * </p>
 */
public class NyARLabelingImage extends NyARRaster_BasicClass {
	...
	/**
	 * コンストラクタです。 ラべリング画像のサイズを指定して、インスタンスを生成します。
	 * 
	 * @param i_width
	 *            ラスタの幅を指定します。
	 * @param i_height
	...
	/**
	 * この関数は、矩形エッジ（画像を囲む枠）を書きます。 ARToolKit互換のコーナー検出の前処理に必要なため、実装しています。
	 */
	 ...
	 * <p>
	 * 格納値 - 値がnullの時はラベル番号そのものがラスタに格納されていますが、 null以外の時はラスタに格納されているのはインデクス番号です。
	 * インデクス番号とラベル番号の関係は、以下の式で表されます。 ラベル番号n,インデクス番号i: n=indextable[i]
	 * </p>
	 * 
	 * @return ラベリング結果がインデックステーブルを持つ場合、その配列を返します。 持たない場合、nullを返します。
	 */
	...
	/**
	 * この関数は、インスタンスの状態をリセットします。 リセット後にインスタンスのラベルスタックはクリアされますが、画像情報は未定義値になります。
	 * 
	...
	public int getTopClipTangentX(NyARLabelingLabel i_label)
		...
		// p1=ShortPointer.wrap(limage,j*xsize+clip.get());//p1
		// =&(limage[j*xsize+clip[0]]);
		for (int i = i_label.clip_l; i <= clip1; i++) {// for( i = clip[0]; i
														// <=clip[1]; i++, p1++
														// ) {
