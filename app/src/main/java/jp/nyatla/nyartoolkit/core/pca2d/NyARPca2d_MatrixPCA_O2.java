...
/**
 * このクラスは、{@link NyARPca2d_MatrixPCA}を展開して高速化したものです。 演算の内容は、
 * {@link NyARPca2d_MatrixPCA}と同一です。
 */
public class NyARPca2d_MatrixPCA_O2 implements INyARPca2d {
	...
	private static void PCA_QRM(NyARDoubleMatrix22 o_matrix, double[] dv)
		...
		dv_x = o_matrix.m00;// this.m[dim - 2][dim - 2];//
							// d.v[dim-2]=a.m[dim-2][dim-2];//d->v[dim-2]=a->m[(dim-2)*dim+(dim-2)];
		ev1 = o_matrix.m01;// this.m[dim - 2][dim - 1];//
							// e.v[dim-2+i_e_start]=a.m[dim-2][dim-1];//e->v[dim-2]
							// = a->m[(dim-2)*dim+(dim-1)];
		dv_y = o_matrix.m11;// this.m[dim - 1][dim - 1];//
		...
		// while (j > 0 && Math.abs(ev1) > PCA_EPS * (Math.abs(dv.x) +
		// Math.abs(dv.y))) {
		// j--;
		// }
		// if (j == 0) {
		...
		do {
			...
			dv_x -= t;// dv->v[k] -= t;
			dv_y += t;// dv->v[k+1] += t;
			ev1 += s * (c * w - 2 * s * ev1);// ev->v[k+1]+= s * (c* w- 2* s
												// *ev->v[k+1]);
	 ...
	 * @throws NyARException
	 */
	private void PCA_PCA(double[] i_v1, double[] i_v2, int i_number_of_data,
			NyARDoubleMatrix22 o_matrix, double[] o_ev, double[] o_mean)
			throws NyARException {
	...
	// override
	public void pca(double[] i_v1, double[] i_v2, int i_number_of_point,
			NyARDoubleMatrix22 o_evec, double[] o_ev, double[] o_mean)
			throws NyARException {
