...
/**
 * このクラスは、矩形情報を格納します。 ARToolKitのARMarkerInfoに相当しますが、このクラスは理想座標のみを取り扱います。
 */
public class NyARSquare {
	...
	/**
	 * この関数は、頂点同士の距離から、頂点のシフト量（回転量）を返します。 よく似た２つの矩形の頂点同士の、頂点の対応を取るために使用します。
	 * 
	 * @param i_square
	 *            比較対象の矩形
	 * @return シフト量を数値で返します。
	 *         シフト量はthis-i_squareです。1の場合、this.sqvertex[0]とi_square.
	 *         sqvertex[1]が対応点になる(shift量1)であることを示します。
	 */
