...
/**
 * このクラスは、NyARToolkitの環境パラメータを格納します。 環境パラメータは、ARToolKitのパラメータと同一です。
...
public class NyARParam {
	 ...
	 * 
	 * @return [read only]歪み補正パラメータオブジェクト
	 */
	...
	/**
	 * この関数は、現在のスクリーンサイズを変更します。 ARToolKitのarParamChangeSize関数に相当します。
	 * 
	 * @param i_xsize
	 *            新しいサイズ
	 * @param i_ysize
	...
	public void changeScreenSize(int i_xsize, int i_ysize) {
		final double scale = (double) i_xsize / (double) (this._screen_size.w);// scale
																				// =
																				// (double)xsize
																				// /
																				// (double)(source->xsize);
	 ...
	 */
	public void makeCameraFrustumRH(double i_dist_min, double i_dist_max,
			NyARDoubleMatrix44 o_frustum) {
		this._projection_matrix.makeCameraFrustumRH(this._screen_size.w,
				this._screen_size.h, i_dist_min, i_dist_max, o_frustum);
		return;
	}
