...
/**
 * このクラスは、グレースケース画像を格納するラスタクラスです。 外部バッファ、内部バッファの両方に対応します。
 */
public class NyARGrayscaleRaster extends NyARRaster_BasicClass {
	...
	/**
	 * コンストラクタです。 内部参照のバッファ（{@link NyARBufferType#INT1D_GRAY_8}
	 * 形式）を持つインスタンスを生成します。
	 * 
	 * @param i_width
	 *            ラスタのサイズ
	...
	/**
	 * コンストラクタです。 画像のサイズパラメータとバッファ参照方式を指定して、インスタンスを生成します。 バッファの形式は、
	 * {@link NyARBufferType#INT1D_GRAY_8}です。
	 * 
	 * @param i_width
	 *            ラスタのサイズ
	 * @param i_height
	 *            ラスタのサイズ
	 * @param i_is_alloc
	 *            バッファを外部参照にするかのフラグ値。 trueなら内部バッファ、falseなら外部バッファを使用します。
	 *            falseの場合、初期のバッファはnullになります。インスタンスを生成したのちに、{@link #wrapBuffer}
	 *            を使って割り当ててください。
	 * @throws NyARException
	 */
	...
	/**
	 * コンストラクタです。 画像のサイズパラメータとバッファ形式を指定して、インスタンスを生成します。
	 * 
	 * @param i_width
	 *            ラスタのサイズ
	 * @param i_height
	 *            ラスタのサイズ
	 * @param i_raster_type
	 *            ラスタのバッファ形式。 {@link NyARBufferType}
	...
	/**
	 * この関数は、ラスタのバッファへの参照値を返します。 バッファの形式は、コンストラクタに指定した形式と同じです。
	 */
	...
	/**
	 * この関数は、インスタンスがバッファを所有するかを返します。 内部参照バッファの場合は、常にtrueです。
	...
	/**
	 * この関数は、指定した数値でラスタを埋めます。 この関数は高速化していません。
	 * 
	 * @param i_value
	 ...
	 * @param o_output
	 *            出力先ラスタ。このラスタの解像度は、w=(i_input.w-i_left)/i_skip,h=(i_input.h-
	 *            i_height)/i_skipを満たす必要があります。 出力先ラスタと入力ラスタの画素形式は、同じである必要があります。
	 */
	public void copyTo(int i_left, int i_top, int i_skip,
			NyARGrayscaleRaster o_output) {
		assert (this.getSize().isInnerSize(i_left + o_output.getWidth()
				* i_skip, i_top + o_output.getHeight() * i_skip));
	...
	protected interface IdoFilterImpl {
		...
		public void copyTo(NyARGrayscaleRaster i_input, int i_left, int i_top,
				int i_skip, NyARGrayscaleRaster o_output);
	}
	...
	protected final class IdoFilterImpl_INT1D_GRAY_8 implements IdoFilterImpl {
		...
		public void copyTo(NyARGrayscaleRaster i_input, int i_left, int i_top,
				int i_skip, NyARGrayscaleRaster o_output) {
			assert (i_input.getSize().isInnerSize(i_left + o_output.getWidth()
					* i_skip, i_top + o_output.getHeight() * i_skip));
			...
			int skip_src_y = (src_size.w - dest_size.w * i_skip) + src_size.w
					* (i_skip - 1);
