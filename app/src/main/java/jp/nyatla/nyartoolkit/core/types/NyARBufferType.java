...
/**
 * このクラスは、バッファの形式を表す定数を定義します。 定数は、主に{@link INyARRaster}で使用する画素形式として使います。
 * バッファ形式定数は32bitの値で、フィールドの組合せで定義しています。 ユーザは、定数からバッファの構造を知ることができます。
 * 
 * <pre>
 * <table>
...
public class NyARBufferType {
	...
	/**
	 * ユーザ定義のバッファ型。USER_DEFINE + (0x0000~0xFFFF) 実験等に使ってください。
	 */
	...
	/**
	 * 　RGB形式。int[1]で、XRGB32の32ビットの画素形式。 (エンディアンはプラットフォーム依存。)
	 */
