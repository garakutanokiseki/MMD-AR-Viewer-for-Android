 ...
 */
final public class NyARRgbPixelReader_BYTE1D_X8R8G8B8_32 implements
		INyARRgbPixelReader {
	/** 参照する外部バッファ */
	 ...
	 *            ラスタのサイズオブジェクトの参照値。
	 */
	public NyARRgbPixelReader_BYTE1D_X8R8G8B8_32(byte[] i_buf,
			NyARIntSize i_size) {
		this._ref_buf = i_buf;
		this._size = i_size;
	}
	...
	public void switchBuffer(Object i_ref_buffer) throws NyARException {
		assert (((byte[]) i_ref_buffer).length >= this._size.w * this._size.h
				* 4);
