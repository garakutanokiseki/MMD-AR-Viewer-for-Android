...
/**
 * このクラスは、参照カウンタ付きのObjectPoolの基本クラスです。 {@link NyARManagedObject}
 ...
 * オブジェクト要素を未割当リストへ加えます。 <br/>
 * このクラスは、NyARManagedObjectと密接に関係して動作することに注意してください。 割り当て関数はオブジェクトプールの
 * {@link NyARManagedObjectPool#newObject}、解放関数にはオブジェクト要素の
 * {@link NyARManagedObject#releaseObject()}を使います。 <br/>
 * また、このクラスは{@link NyARManagedObject}に対して、特殊な関数を持つインタフェイスを提供します。 このインタフェイスは、
 * {@link NyARManagedObject}が{@link NyARManagedObjectPool}のリストを操作するために使います。
 * <p>
 * 継承クラスの実装について - このクラスは、そのまま実体化できません。継承クラスを実装する必要があります。 実装するには、
 * {@link #createElement}をオーバライドして、コンストラクタから{@link #initInstance}を呼び出します。
...
public class NyARManagedObjectPool<T extends NyARManagedObject> {
	/**
	 * このクラスは、{@link NyARManagedObject}へ提供する操作インタフェイスの実体です。 未割当リストを操作する関数を定義します。
	...
	/**
	 * コンストラクタです。 生成拒否の為に、コンストラクタを隠蔽します。 継承クラスを作成してください。
	 */
	protected NyARManagedObjectPool() {
	}

	/**
	 * この関数は、インスタンスを初期化します。 継承クラスのコンストラクタから呼び出します。
	...
	/**
	 * この関数は、配列要素のオブジェクトを１個作ります。 {@link #initInstance(int, Class)}から呼び出されます。
	 * 継承クラスでオーバライドして、要素オブジェクトを１個生成して返す処理を実装してください。
	 * 
	 * @return 新しいオブジェクトを返してください。
	 * @throws NyARException
	 */
