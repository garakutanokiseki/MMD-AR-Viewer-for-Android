...
public class NyARTransMat_ARToolKit implements INyARTransMat {
	 ...
	 */
	public NyARTransMat_ARToolKit(NyARCameraDistortionFactor i_ref_distfactor,
	...
	private void initInstance(NyARCameraDistortionFactor i_ref_distfactor,
			NyARPerspectiveProjectionMatrix i_ref_projmat) throws NyARException {
	 ...
	 */
	public final void transMat(NyARSquare i_square, NyARRectOffset i_offset,
		...
		double err = this.optimize(this._rotmatrix, trans, this._transsolver,
				i_offset.vertex, vertex_2d);
	...
	public final void transMatContinue(NyARSquare i_square,
		...
		if (err > AR_GET_TRANS_CONT_MAT_MAX_FIT_ERROR) {
			...
			final double err2 = this.optimize(this._rotmatrix, trans,
					this._transsolver, i_offset.vertex, vertex_2d);
			// エラー値が低かったら値を差換え
			if (err2 < err) {
				// 良い値が取れたら、差換え
	...
	private double optimize(NyARRotMatrix_ARToolKit io_rotmat,
			NyARDoublePoint3d io_transvec, INyARTransportVectorSolver i_solver,
		...
		for (int i = 0;; i++) {
			// <arGetTransMat3>
			err = this._mat_optimize.modifyMatrix(io_rotmat, io_transvec,
					i_offset_3d, i_2d_vertex);
			...
			if (err < AR_GET_TRANS_MAT_MAX_FIT_ERROR
					|| i == AR_GET_TRANS_MAT_MAX_LOOP_COUNT - 1) {
				break;
			}
			io_rotmat.getPoint3dBatch(i_offset_3d, vertex_3d, 4);
			i_solver.solveTransportVector(vertex_3d, io_transvec);
