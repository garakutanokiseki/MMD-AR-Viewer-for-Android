 ...
 * 評価アルゴリズムは、ARToolKitの、AR_TEMPLATE_MATCHING_BWと同様です。 比較対象のデータには、
 * {@link NyARMatchPattDeviationBlackWhiteData}クラスの物を使います。
 */
public class NyARMatchPatt_BlackWhite implements INyARMatchPatt {
	 ...
	 * @throws NyARException
	 */
	public boolean evaluate(NyARMatchPattDeviationBlackWhiteData i_patt,
		...
		for (int j = 0; j < 4; j++) {
			// 合計値初期化
			sum = 0;
			final NyARMatchPattDeviationBlackWhiteData code_patt = this._ref_code_patt
					.getBlackWhiteData(j);
			...
			final double sum2 = sum / code_patt.getPow() / i_patt.getPow();// sum2
																			// =
																			// sum
																			// /
																			// patpow[k][j]/
