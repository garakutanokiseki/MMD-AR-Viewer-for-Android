...
/**
 * このクラスは、座標配列を直線式に変換します。 座標配列の連続する要素を主成分分析にかけて、直線式にします。
 */
public class NyARCoord2Linear {
	...
	/**
	 * コンストラクタです。 輪郭取得元画像の歪み矯正オブジェクトとサイズを指定して、インスタンスを生成します。
	 * 
	 * @param i_size
	 *            入力画像のサイズ
	 * @param i_distfactor
	 *            樽型歪みを補正する場合に、オブジェクトを指定します。 nullの場合、補正を行いません。
	 */
	public NyARCoord2Linear(NyARIntSize i_size,
			NyARCameraDistortionFactor i_distfactor) {
	 ...
	 * @param o_line
	 *            直線式を受け取るオブジェクト
	 * @return 直線式の計算に成功すると、trueを返します。
	 * @throws NyARException
	 */
	public boolean coord2Line(int i_st, int i_ed, NyARIntCoordinates i_coord,
		...
		if (st <= ed) {
			...
			if (this._dist_factor != null) {
				this._dist_factor.observ2IdealBatch(i_coord.items, st, n,
			...
			n = ed + 1 + cood_num - st;
			if (this._dist_factor != null) {
				this._dist_factor.observ2IdealBatch(i_coord.items, st, cood_num
						- st, this._xpos, this._ypos, 0);
				this._dist_factor.observ2IdealBatch(i_coord.items, 0, ed + 1,
						this._xpos, this._ypos, cood_num - st);
			}
		}
		// 要素数の確認
		if (n < 2) {
		...
		this._pca.pca(this._xpos, this._ypos, n, evec, this.__getSquareLine_ev,
				mean);
		o_line.a = evec.m01;// line[i][0] = evec->m[1];
		o_line.b = -evec.m00;// line[i][1] = -evec->m[0];
		o_line.c = -(o_line.a * mean[0] + o_line.b * mean[1]);// line[i][2] =
																// -(line[i][0]*mean->v[0]
																// +
																// line[i][1]*mean->v[1]);
