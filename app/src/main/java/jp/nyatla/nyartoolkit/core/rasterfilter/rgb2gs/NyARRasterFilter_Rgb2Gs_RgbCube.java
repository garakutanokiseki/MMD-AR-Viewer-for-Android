 ...
 * <p>
 * アルゴリズム このフィルタは、RGB値の平均値を、(R*G*B)/(255*255)で算出します。(スケールは、255>=n>=0になります。)
 * 三乗根ではないことに注意してください。
 * </p>
 * <p>
 ...
 * </p>
 */
public class NyARRasterFilter_Rgb2Gs_RgbCube implements
		...
		public void doFilter(INyARRaster i_input, INyARRaster i_output,
			...
			for (int y = 0; y < i_size.h; y++) {
				for (int x = 0; x < i_size.w; x++) {
					out_buf[y * i_size.w + x] = ((in_buf[bp] & 0xff)
							* (in_buf[bp + 1] & 0xff) * (in_buf[bp + 2] & 0xff)) >> 16;
					bp += 3;
				}
			}
			return;
