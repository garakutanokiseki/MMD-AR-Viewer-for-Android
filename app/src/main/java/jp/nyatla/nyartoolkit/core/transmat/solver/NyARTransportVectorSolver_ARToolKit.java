 ...
 */
public class NyARTransportVectorSolver_ARToolKit implements
	...
	/**
	 * コンストラクタです。 射影変換オブジェクトの参照値を指定して、インスタンスを生成します。
	 * 
	 * @param i_projection_mat_ref
	 *            射影変換オブジェクトの参照値です。
	 */
	public NyARTransportVectorSolver_ARToolKit(
		...
		for (int i = 0; i < 4; i++) {
			final int x2 = i * 2;
			mata[x2][0] = matat[0][x2] = i_projection_mat_ref.m00;// mat_a->m[j*6+0]=mat_b->m[num*0+j*2]
																	// =cpara[0][0];
			mata[x2][1] = matat[1][x2] = i_projection_mat_ref.m01;// mat_a->m[j*6+1]=mat_b->m[num*2+j*2]=cpara[0][1];
			mata[x2 + 1][0] = matat[0][x2 + 1] = 0.0;// mat_a->m[j*6+3]
														// =mat_b->m[num*0+j*2+1]=
														// 0.0;
			mata[x2 + 1][1] = matat[1][x2 + 1] = i_projection_mat_ref.m11;// mat_a->m[j*6+4]
																			// =mat_b->m[num*2+j*2+1]=
																			// cpara[1][1];
		}
		return;
	...
	/**
	 * この関数は、射影変換後の２次元頂点座標をセットします。 i_number_of_vertexは４である必要があります。
	 */
	...
	public void solveTransportVector(NyARDoublePoint3d[] i_vertex3d,
		...
		for (int i = 0; i < 4; i++) {
			...
			// 透視変換？
			matc[x2][0] = point3d_ptr.z * cx[i] - cpara00 * point3d_ptr.x
					- cpara01 * point3d_ptr.y - cpara02 * point3d_ptr.z;// mat_c->m[j*2+0]
																		// =
																		// wz*pos2d[j][0]-cpara[0][0]*wx-cpara[0][1]*wy-cpara[0][2]*wz;
			matc[x2 + 1][0] = point3d_ptr.z * cy[i] - cpara11 * point3d_ptr.y
					- cpara12 * point3d_ptr.z;// mat_c->m[j*2+1]=
												// wz*pos2d[j][1]-cpara[1][1]*wy-cpara[1][2]*wz;
		}
