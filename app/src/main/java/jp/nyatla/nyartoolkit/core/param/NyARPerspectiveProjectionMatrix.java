...
/**
 * このクラスは、ARToolKit形式の透視変換行列を格納します。 透視変換関数と、射影変換行列の生成関数を提供します。 このクラスは
 * {@link NyARParam}に所有されることを前提にしており、単独の仕様は考慮されていません。
 * <p>
 * アルゴリズム -
...
final public class NyARPerspectiveProjectionMatrix extends NyARDoubleMatrix44 {
	...
	/*
	 * static double dot( double a1, double a2, double a3,double b1, double
	 * b2,double b3 )
	 */
	...
	/**
	 * この関数は、ARToolKitのarParamDecompMatと同じです。 動作はよくわかりません…。
	 * 
	 * @param o_cpara
	...
	public void decompMat(NyARMat o_cpara, NyARMat o_trans) {
		...
		cpara[2][2] = norm(c20, c21, c22);// cpara[2][2] =norm(
											// Cpara[2][0],Cpara[2][1],Cpara[2][2]);
		trans[2][0] = c20 / cpara[2][2];// trans[2][0] = Cpara[2][0]
										// /cpara[2][2];
		trans[2][1] = c21 / cpara[2][2];// trans[2][1] = Cpara[2][1] /
										// cpara[2][2];
		trans[2][2] = c22 / cpara[2][2];// trans[2][2] =Cpara[2][2]
										// /cpara[2][2];
		trans[2][3] = c23 / cpara[2][2];// trans[2][3] =Cpara[2][3]
										// /cpara[2][2];
		...
		rem1 = c10 - cpara[1][2] * trans[2][0];// rem1 =Cpara[1][0] -cpara[1][2]
												// *trans[2][0];
		rem2 = c11 - cpara[1][2] * trans[2][1];// rem2 =Cpara[1][1] -cpara[1][2]
												// *trans[2][1];
		rem3 = c12 - cpara[1][2] * trans[2][2];// rem3 =Cpara[1][2] -cpara[1][2]
												// *trans[2][2];
		...
		cpara[0][2] = dot(trans[2][0], trans[2][1], trans[2][2], c00, c01, c02);// cpara[0][2]
																				// =dot(trans[2][0],
																				// trans[2][1],trans[2][2],Cpara[0][0],Cpara[0][1],Cpara[0][2]);
		...
		rem1 = c00 - cpara[0][1] * trans[1][0] - cpara[0][2] * trans[2][0];// rem1
																			// =
																			// Cpara[0][0]
																			// -
																			// cpara[0][1]*trans[1][0]-
																			// cpara[0][2]*trans[2][0];
		rem2 = c01 - cpara[0][1] * trans[1][1] - cpara[0][2] * trans[2][1];// rem2
																			// =
																			// Cpara[0][1]
																			// -
																			// cpara[0][1]*trans[1][1]-
																			// cpara[0][2]*trans[2][1];
		rem3 = c02 - cpara[0][1] * trans[1][2] - cpara[0][2] * trans[2][2];// rem3
																			// =
																			// Cpara[0][2]
																			// -
																			// cpara[0][1]*trans[1][2]
																			// -
																			// cpara[0][2]*trans[2][2];
		cpara[0][0] = norm(rem1, rem2, rem3);// cpara[0][0] = norm( rem1,rem2,
												// rem3 );
		...
		trans[1][3] = (c13 - cpara[1][2] * trans[2][3]) / cpara[1][1];// trans[1][3]
																		// =
																		// (Cpara[1][3]
																		// -cpara[1][2]*trans[2][3])
																		// /
																		// cpara[1][1];
		trans[0][3] = (c03 - cpara[0][1] * trans[1][3] - cpara[0][2]
				* trans[2][3])
				/ cpara[0][0];// trans[0][3] = (Cpara[0][3]
								// -cpara[0][1]*trans[1][3]-cpara[0][2]*trans[2][3])
								// / cpara[0][0];
	...
	public void changeScale(double i_scale) {
		...
		// for (int i = 0; i < 4; i++) {
		// array34[0 * 4 + i] = array34[0 * 4 + i] * scale;//
		// newparam->mat[0][i]=source->mat[0][i]* scale;
		// array34[1 * 4 + i] = array34[1 * 4 + i] * scale;//
	 ...
	 *            変換後の座標値を受け取るオブジェクト
	 */
	public final void project(NyARDoublePoint3d i_3dvertex,
			NyARDoublePoint2d o_2d) {
		double w = 1 / (i_3dvertex.z * this.m22);
		o_2d.x = (i_3dvertex.x * this.m00 + i_3dvertex.y * this.m01 + i_3dvertex.z
				* this.m02)
				* w;
	 ...
	 *            変換後の座標値を受け取るオブジェクト
	 */
	public final void project(double i_x, double i_y, double i_z,
			NyARDoublePoint2d o_2d) {
	...
	public final void project(NyARDoublePoint3d i_3dvertex, NyARIntPoint2d o_2d) {
		double w = 1 / (i_3dvertex.z * this.m22);
		o_2d.x = (int) ((i_3dvertex.x * this.m00 + i_3dvertex.y * this.m01 + i_3dvertex.z
				* this.m02) * w);
	...
	/**
	 * 右手系の視錐台を作ります。 この視錐台は、ARToolKitのarglCameraViewRHの作る視錐台と同じです。
	 * 
	 ...
	 *            視錐台の格納先オブジェクトを指定します。
	 */
	public void makeCameraFrustumRH(double i_screen_width,
			double i_screen_height, double i_dist_min, double i_dist_max,
		...
		for (i = 0; i < 4; i++) {
			icpara[1][i] = (i_screen_height - 1) * (icpara[2][i])
					- icpara[1][i];
		}
		...
		q03 = 0.0;
		o_frustum.m00 = q00 * trans[0][0] + q01 * trans[1][0] + q02
				* trans[2][0];
		o_frustum.m01 = q00 * trans[0][1] + q01 * trans[1][1] + q02
				* trans[2][1];
		o_frustum.m02 = q00 * trans[0][2] + q01 * trans[1][2] + q02
				* trans[2][2];
		o_frustum.m03 = q00 * trans[0][3] + q01 * trans[1][3] + q02
				* trans[2][3] + q03;
		...
		q13 = 0.0;
		o_frustum.m10 = q10 * trans[0][0] + q11 * trans[1][0] + q12
				* trans[2][0];
		o_frustum.m11 = q10 * trans[0][1] + q11 * trans[1][1] + q12
				* trans[2][1];
		o_frustum.m12 = q10 * trans[0][2] + q11 * trans[1][2] + q12
				* trans[2][2];
		o_frustum.m13 = q10 * trans[0][3] + q11 * trans[1][3] + q12
				* trans[2][3] + q13;
		...
		o_frustum.m20 = q20 * trans[0][0] + q21 * trans[1][0] + q22
				* trans[2][0];
		o_frustum.m21 = q20 * trans[0][1] + q21 * trans[1][1] + q22
				* trans[2][1];
		o_frustum.m22 = q20 * trans[0][2] + q21 * trans[1][2] + q22
				* trans[2][2];
		o_frustum.m23 = q20 * trans[0][3] + q21 * trans[1][3] + q22
				* trans[2][3] + q23;
		...
		q32 = -1.0;
		q33 = 0.0;
		o_frustum.m30 = q30 * trans[0][0] + q31 * trans[1][0] + q32
				* trans[2][0];
		o_frustum.m31 = q30 * trans[0][1] + q31 * trans[1][1] + q32
				* trans[2][1];
		o_frustum.m32 = q30 * trans[0][2] + q31 * trans[1][2] + q32
				* trans[2][2];
		o_frustum.m33 = q30 * trans[0][3] + q31 * trans[1][3] + q32
				* trans[2][3] + q33;
		return;
	}
}
