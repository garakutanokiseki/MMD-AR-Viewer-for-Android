...
/**
 * このクラスは、{@link NyARLabeling_Rle}クラスを用いた矩形検出器です。 検出した矩形を、自己コールバック関数
 * {@link #onSquareDetect}へ通知します。 継承クラスで自己コールバック関数{@link #onSquareDetect}
 * を実装する必要があります。
 */
public abstract class NyARSquareContourDetector_Rle extends
	...
	private class Labeling extends NyARLabeling_Rle {
		...
		public Labeling(int i_width, int i_height) throws NyARException {
			...
			this.label_stack = new NyARRleLabelFragmentInfoPtrStack(i_width
					* i_height * 2048 / (320 * 240) + 32);// 検出可能な最大ラベル数
			this._bottom = i_height - 1;
			this._right = i_width - 1;
			return;
		}

		public void labeling(NyARGrayscaleRaster i_raster, NyARIntRect i_area,
	...
	private final NyARLabelOverlapChecker<NyARRleLabelFragmentInfo> _overlap_checker = new NyARLabelOverlapChecker<NyARRleLabelFragmentInfo>(
			32, NyARRleLabelFragmentInfo.class);
	 ...
	 * 
	 * @param i_raster
	 *            検出元のラスタ画像 入力できるラスタの画素形式は、
	 *            {@link NyARLabeling_Rle#labeling(NyARGrayscaleRaster, int)}
	 *            と同じです。
	 * @param i_area
	 ...
	 * @throws NyARException
	 */
	public void detectMarker(NyARGrayscaleRaster i_raster, NyARIntRect i_area,
			int i_th) throws NyARException {
		...
		for (int i = 0; i < label_num; i++) {
			...
			// 輪郭を取得
			if (!this._cpickup.getContour(i_raster, i_area, i_th,
					label_pt.entry_x, label_pt.clip_t, coord)) {
				continue;
			}
			int label_area = label_pt.area;
			// 輪郭線をチェックして、矩形かどうかを判定。矩形ならばmkvertexに取得
			if (!this._coord2vertex.getVertexIndexes(coord, label_area,
					mkvertex)) {
				// 頂点の取得が出来なかった
				continue;
			}
	...
	public void detectMarker(NyARBinRaster i_raster) throws NyARException {
		...
		for (int i = 0; i < label_num; i++) {
			...
			// 輪郭を取得
			if (!this._cpickup.getContour(i_raster, label_pt.entry_x,
					label_pt.clip_t, coord)) {
				continue;
			}
			// 輪郭線をチェックして、矩形かどうかを判定。矩形ならばmkvertexに取得
			if (!this._coord2vertex.getVertexIndexes(coord, label_area,
