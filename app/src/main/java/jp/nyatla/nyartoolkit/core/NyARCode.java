...
/**
 * {@link NyARCode}クラスの支援クラスです。 このクラスは、{@link NyARCode}
 * のマーカファイル読み取り機能のうち、InputStreamからARToolkit形式のマーカデータを読み取って配列に格納する手順を実装します。
...
class NyARCodeFileReader {
	 ...
	 */
	public static void loadFromARToolKitFormFile(InputStream i_stream,
			NyARRaster[] o_raster) throws NyARException {
		...
		try {
			...
			for (int h = 0; h < 4; h++) {
				...
				readBlock(st, ra.getWidth(), ra.getHeight(),
						(int[]) ra.getBuffer());
			}
		} catch (Exception e) {
	 ...
	 */
	public static void loadFromARToolKitFormFile(InputStream i_stream,
			NyARCode o_code) throws NyARException {
		int width = o_code.getWidth();
		int height = o_code.getHeight();
		NyARRgbRaster tmp_raster = new NyARRgbRaster(width, height,
	 ...
	 * 
	 * @param i_st
	 *            入力元のStreamTokenizerを指定します。 i_stの読み取り位置は更新されます。
	 * @param i_width
	 *            パターンの横解像度(pixel)です。
	 * @param i_height
	 *            パターンの縦解像度(pixel)です。
	 ...
	 * @throws NyARException
	 */
	private static void readBlock(StreamTokenizer i_st, int i_width,
			int i_height, int[] o_buf) throws NyARException {
		try {
			...
			for (int i3 = 0; i3 < 3; i3++) {
				for (int i2 = 0; i2 < pixels; i2++) {
					...
					case StreamTokenizer.TT_NUMBER:
						break;
					default:
						throw new NyARException(i_st.toString());
					}
					o_buf[i2] = (o_buf[i2] << 8)
							| ((0x000000ff & (int) i_st.nval));
				}
			}
			// GBR→RGB
			for (int i3 = 0; i3 < pixels; i3++) {
				o_buf[i3] = ((o_buf[i3] << 16) & 0xff0000)
						| (o_buf[i3] & 0x00ff00)
						| ((o_buf[i3] >> 16) & 0x0000ff);
			}
		} catch (Exception e) {
			throw new NyARException(e);
		}
...
/**
 * このクラスは、ARToolKitのマーカーパターン1個のデータを格納します。 マーカーパターンのプロパティと、データのロード機能を提供します。
 */
public class NyARCode {
	 ...
	 * 
	 * @param i_index
	 *            方位インデクスの値を指定します。 範囲は、0&lt;=n&lt;=3の数値です。
	 * @return 指定した方位の{@link NyARMatchPattDeviationColorData}オブジェクトを返します。
	 */
	...
	/**
	 * 指定したdirection(方位)の{@link NyARMatchPattDeviationBlackWhiteData}
	 * オブジェクトの参照値を返します。
	 * 
	 * @param i_index
	 *            方位インデクスの値を指定します。 範囲は、0&lt;=n&lt;=3の数値です。
	 * @return 指定した方位の{@link NyARMatchPattDeviationBlackWhiteData}オブジェクトを返します。
	 */
	...
	public NyARCode(int i_width, int i_height) throws NyARException {
		...
		// 空のラスタを4個作成
		for (int i = 0; i < 4; i++) {
			this._color_pat[i] = new NyARMatchPattDeviationColorData(i_width,
					i_height);
			this._bw_pat[i] = new NyARMatchPattDeviationBlackWhiteData(i_width,
					i_height);
		}
		return;
	}
	...
	/**
	 * 1枚のラスタオブジェクトから、マーカーパターンを生成して格納します。 残りの3枚のデータは、関数がi_rasterを回転させて求めます。
	 * 
	 * @param i_raster
	 *            パターンデータを格納したラスタオブジェクトを指定します。 ラスタは、このインスタンスと同じ解像度である必要があります。
	 * @throws NyARException
	 */
	public void setRaster(INyARRgbRaster i_raster) throws NyARException {
		// ラスタにパターンをロードする。
