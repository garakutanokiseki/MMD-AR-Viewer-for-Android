...
/**
 * このクラスは、樽型歪み設定/解除クラスです。 パラメータには、ARToolKitの樽型歪みパラメータを使います。
 * <p>
 * アルゴリズム - このクラスでは、歪み矯正前の座標を観察座標系、歪み矯正後の座標を理想座標系と呼びます。
...
public class NyARCameraDistortionFactor {
	...
	/**
	 * この関数は、歪みパラメータをスケール倍します。 パラメータ値は、スケール値の大きさだけ、拡大、又は縮小します。
	 * 
	 * @param i_scale
	 *            パラメータの倍率。
	 */
	public void changeScale(double i_scale) {
		...
		this._f2 = this._f2 / (i_scale * i_scale);// newparam->dist_factor[2]=source->dist_factor[2]/
													// (scale*scale);
	 ...
	 *            変換後の座標を受け取るオブジェクト
	 */
	public final void ideal2Observ(NyARDoublePoint2d i_in,
	 ...
	 *            変換する座標の個数。
	 */
	public final void ideal2ObservBatch(NyARDoublePoint2d[] i_in,
			NyARDoublePoint2d[] o_out, int i_size) {
		double x, y;
	 ...
	 */
	public final void ideal2ObservBatch(NyARDoublePoint2d[] i_in,
			NyARIntPoint2d[] o_out, int i_size) {
		double x, y;
		final double d0 = this._f0;
		final double d1 = this._f1;
		final double d3 = this._f3;
	 ...
	 *            変換後の座標を受け取るオブジェクト
	 */
	public final void observ2Ideal(double ix, double iy,
	 ...
	 *            変換元の座標
	 * @param o_veclinear
	 *            変換後の座標を受け取るオブジェクト。{@link NyARVecLinear2d#x}と
	 *            {@link NyARVecLinear2d#y}のみに値をセットします。
	 */
