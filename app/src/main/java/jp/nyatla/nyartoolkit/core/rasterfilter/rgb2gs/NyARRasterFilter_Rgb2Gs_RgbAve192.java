 ...
 * <p>
 * アルゴリズム このフィルタは、RGB値の平均値を、(R+G+B)>>4で算出します。(スケールは、191>=n>=0になります。)
 ...
 * </p>
 */
public class NyARRasterFilter_Rgb2Gs_RgbAve192 implements
		INyARRasterFilter_Rgb2Gs {
	/** 変換フィルタのインスタンス */
	protected IdoThFilterImpl _do_filter_impl;

	/**
	 * コンストラクタです。 入力、出力ラスタの画素形式を指定して、フィルタを作成します。
	 * 
	...
	public NyARRasterFilter_Rgb2Gs_RgbAve192(int i_in_raster_type,
			int i_out_raster_type) throws NyARException {
	...
	/**
	 * コンストラクタです。 入力ラスタの画素形式を指定して、フィルタを作成します。 出力ラスタの形式は、
	 * {@link NyARBufferType#INT1D_GRAY_8}を選択します。
	 * 
	 * @param i_in_raster_type
	 *            入力ラスタの形式です。
	 * @throws NyARException
	...
	/**
	 * この関数は、入力画像をグレースケール化して出力画像へ書込みます。 入力画像と出力画像のサイズは同じである必要があります。
	 */
	public void doFilter(INyARRgbRaster i_input, NyARGrayscaleRaster i_output)
			throws NyARException {
		assert (i_input.getSize().isEqualSize(i_output.getSize()) == true);
		NyARIntSize s = i_input.getSize();
		this._do_filter_impl.doFilter(i_input, (int[]) i_output.getBuffer(), 0,
				0, s.w, s.h);
		return;
	}

	/**
	 * この関数は、入力画像の一部分だけをグレースケール化して、出力画像の該当位置へ書込みます。 入力画像と出力画像のサイズは同じである必要があります。
	 * 
	 * @param i_input
	 *            入力画像
	 * @param i_rect
	 ...
	 * @throws NyARException
	 */
	public void doFilter(INyARRgbRaster i_input, NyARIntRect i_rect,
			NyARGrayscaleRaster i_output) throws NyARException {
		assert (i_input.getSize().isEqualSize(i_output.getSize()) == true);
		this._do_filter_impl.doFilter(i_input, (int[]) i_output.getBuffer(),
				i_rect.x, i_rect.y, i_rect.w, i_rect.h);
	 ...
	 *            出力画像です。
	 */
	public void doCutFilter(INyARRgbRaster i_input, int i_left, int i_top,
			int i_skip, NyARGrayscaleRaster i_output) throws NyARException {
		this._do_filter_impl.doCutFilter(i_input, i_left, i_top, i_skip,
				i_output);
	}
	...
	protected interface IdoThFilterImpl {
		 ...
		 * @param h
		 */
		public void doFilter(INyARRaster i_input, int[] o_output, int l, int t,
				int w, int h) throws NyARException;
		 ...
		 * @param o_output
		 */
		public void doCutFilter(INyARRaster i_input, int l, int t, int i_st,
				NyARGrayscaleRaster o_output) throws NyARException;
	}
	...
	private class doThFilterImpl_BUFFERFORMAT_INT1D_X8R8G8B8_32 implements
		...
		public void doCutFilter(INyARRaster i_input, int l, int t, int i_st,
			...
			assert (i_input.getSize().isInnerSize(l + o_output.getWidth()
					* i_st, t + o_output.getHeight() * i_st));
			...
			int skip_src_y = (src_size.w - dest_size.w * i_st) + src_size.w
					* (i_st - 1);
			int skip_src_x = i_st;
	...
	private class doThFilterImpl_BYTE1D_B8G8R8_24 implements IdoThFilterImpl {
		public void doCutFilter(INyARRaster i_input, int l, int t, int i_st,
				NyARGrayscaleRaster o_output) throws NyARException {
			assert (i_input.isEqualBufferType(NyARBufferType.BYTE1D_B8G8R8_24) || i_input
			...
			NyARIntSize dest_size = o_output.getSize();
			NyARIntSize src_size = i_input.getSize();
			int skip_src_y = (src_size.w - dest_size.w * i_st) * 3 + src_size.w
					* (i_st - 1) * 3;
			...
			for (int y = dest_size.h - 1; y >= 0; y -= 1) {
				...
				for (x = pix_count - 1; x >= pix_mod_part; x--) {
					output[pt_dst++] = ((input[pt_src + 0] & 0xff)
							+ (input[pt_src + 1] & 0xff) + (input[pt_src + 2] & 0xff)) >> 2;
					pt_src += skip_src_x;
				}
				for (; x >= 0; x -= 8) {
					output[pt_dst++] = ((input[pt_src + 0] & 0xff)
		...
		public void doFilter(INyARRaster i_input, int[] o_output, int l, int t,
			...
			for (int y = t; y < b; y++) {
				...
				int x = 0;
				for (x = pix_count - 1; x >= pix_mod_part; x--) {
					o_output[src_ptr++] = ((in_buf[bp++] & 0xff)
							+ (in_buf[bp++] & 0xff) + (in_buf[bp++] & 0xff)) >> 2;
				}
				for (; x >= 0; x -= 8) {
					o_output[src_ptr++] = ((in_buf[bp++] & 0xff)
							+ (in_buf[bp++] & 0xff) + (in_buf[bp++] & 0xff)) >> 2;
	...
	private class doThFilterImpl_BYTE1D_B8G8R8X8_32 implements IdoThFilterImpl {
		...
		public void doFilter(INyARRaster i_input, int[] o_output, int l, int t,
			...
			for (int y = t; y < b; y++) {
				for (int x = 0; x < w; x++) {
					o_output[y * size.w + x + l] = ((in_buf[bp] & 0xff)
							+ (in_buf[bp + 1] & 0xff) + (in_buf[bp + 2] & 0xff)) >> 2;
					bp += 4;
				}
				bp += row_padding;
			}
