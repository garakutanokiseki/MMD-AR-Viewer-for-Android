 ...
 * </p>
 */
public class NyARRasterFilter_EqualizeHist extends
		NyARRasterFilter_CustomToneTable {
	private NyARRasterAnalyzer_Histogram _hist_analyzer;
	private NyARHistogram _histogram = new NyARHistogram(256);

	/**
	 * コンストラクタです。 入出力ラスタの形式と、ヒストグラムのサンプリングパラメータを入力して、インスタンスを生成します。
	 * 
	 * @param i_raster_type
	 *            入出力ラスタの画素形式。
	 * @param i_sample_interval
	 *            ヒストグラム分析の間引き値。何ラインおきに画素のスキャンを行うかを指定します。 1以上の数値を指定してください。
	 * @throws NyARException
	 */
	public NyARRasterFilter_EqualizeHist(int i_raster_type,
			int i_sample_interval) throws NyARException {
		super(i_raster_type);
		this._hist_analyzer = new NyARRasterAnalyzer_Histogram(i_raster_type,
				i_sample_interval);
	}

	/**
	 * 入力ラスタにトーンフィルタを適応した画素を出力ラスタへ書込みます。 画素形式は、コンストラクタに指定した形式に合せてください。
	 */
