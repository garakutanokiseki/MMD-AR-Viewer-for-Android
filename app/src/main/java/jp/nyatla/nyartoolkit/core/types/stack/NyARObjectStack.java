...
/**
 * このクラスは、オブジェクトを格納する可変長配列です。 配列に、オブジェクトの実態を所有します。
 * このクラスの実体化は禁止しています。継承して使ってください。
 * <p>
 * 継承クラスの実装方法 - 配列要素の生成シーケンスを実装するには、{@link #createElement}をオーバライドして、 コンストラクタから
 * {@link #initInstance}を呼び出します。 {@link #initInstance}
 * には２種類の関数があります。要素生成パラメータの有無で、どちらかを選択して呼び出してください。
 * </p>
 * 
 * @param <T>
 *            配列型を指定します。
...
public class NyARObjectStack<T> extends NyARPointerStack<T> {
	...
	/**
	 * この関数は、配列から新しい要素を１個わりあてて返します。 関数が成功すると、有効な配列長が+1されます。
	 * 
	 * @return 成功すると、新しい配列要素。 失敗するとnull
	 * @throws NyARException
	 */
