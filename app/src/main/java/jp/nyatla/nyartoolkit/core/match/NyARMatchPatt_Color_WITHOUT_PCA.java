...
/**
 * このクラスは、カラーで２パターンの一致度を計算します。 評価アルゴリズムは、ARToolKitの、
 * AR_TEMPLATE_MATCHING_COLORかつAR_MATCHING_WITHOUT_PCAと同様です。 比較対象のデータには、
 * {@link NyARMatchPattDeviationColorData}クラスの物を使います。
 */
public class NyARMatchPatt_Color_WITHOUT_PCA implements INyARMatchPatt {
	...
	/**
	 * コンストラクタ。 基準パターンを元に、インスタンスを生成します。
	 * 
	 * @param i_code_ref
	 *            セットする基準パターンの参照値
	 */
	...
	public boolean evaluate(NyARMatchPattDeviationColorData i_patt,
		...
		for (int j = 0; j < 4; j++) {
			// 合計値初期化
			sum = 0;
			final NyARMatchPattDeviationColorData code_patt = this._code_patt
					.getColorData(j);
			...
			// <全画素について、比較(FORの1/16展開)/>
			final double sum2 = sum / code_patt.getPow();// sum2 = sum /
															// patpow[k][j]/
															// datapow;
			if (sum2 > max) {
				max = sum2;
