 ...
 * オブジェクトの有効性を判断するための、参照カウンタをもちます。 {@link NyARManagedObjectPool}
 * に対して、オブジェクトの操作インタフェイスを提供します。
 */
public class NyARManagedObject {
	...
	/**
	 * コンストラクタです。 所有される{@link NyARManagedObjectPool}を指定して、インスタンスを作成します。 この関数は、
	 * {@link NyARManagedObjectPool#createElement}関数が呼び出します。ユーザが使うことはありません。
	 * 
	 * @param i_ref_pool_operator
	 *            このオブジェクトの所有者の持つ、操作インタフェイス
	 */
	protected NyARManagedObject(
			INyARManagedObjectPoolOperater i_ref_pool_operator) {
		this._count = 0;
		this._pool_operater = i_ref_pool_operator;
	}

	/**
	 * この関数は、オブジェクトを初期状態にします。 この関数は、{@link NyARManagedObjectPool}
	 * が呼び出します。ユーザが呼び出すことはありません。
	 * 
	 * @return このオブジェクトを初期化したオブジェクト。
	 */
	...
	/**
	 * この関数は、オブジェクトの参照カウンタを1減算します。 参照カウンタが0になると、オブジェクトは未参照状態となり、自動的に
	 * {@link NyARManagedObjectPool}へ返却されます。
	 * 
	 * @return 減算後の参照カウンタ
	 */
