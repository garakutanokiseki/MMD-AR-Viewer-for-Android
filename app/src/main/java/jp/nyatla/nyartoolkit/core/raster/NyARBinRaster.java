...
/**
 * このクラスは、２値画像を格納するラスタクラスです。 外部バッファ、内部バッファの両方に対応します。
 */
public class NyARBinRaster extends NyARRaster_BasicClass {
	 ...
	 */
	public NyARBinRaster(int i_width, int i_height, int i_raster_type,
	...
	/**
	 * コンストラクタです。 画像のサイズパラメータを指定して、{@link NyARBufferType#INT2D_BIN_8}
	 * 形式のバッファを持つインスタンスを生成します。
	 * 
	 * @param i_width
	 *            ラスタのサイズ
	 * @param i_height
	 ...
	 * @throws NyARException
	 */
	public NyARBinRaster(int i_width, int i_height, boolean i_is_alloc)
			throws NyARException {
	...
	/**
	 * コンストラクタです。 画像のサイズパラメータを指定して、{@link NyARBufferType#INT2D_BIN_8}
	 * 形式のバッファを持つインスタンスを生成します。 このラスタは、内部参照バッファを持ちます。
	 * 
	 * @param i_width
	 *            ラスタのサイズ
	 * @param i_height
	...
	/**
	 * この関数は、インスタンスの初期化シーケンスを実装します。 コンストラクタから呼び出します。
	 * 
	 * @param i_size
	 *            ラスタのサイズ
	 * @param i_buf_type
	...
	protected boolean initInstance(NyARIntSize i_size, int i_buf_type,
		...
		case NyARBufferType.INT1D_BIN_8:
			int size = i_size.w * i_size.h;
			if(i_is_alloc || prevSize!=size){
				_buf = new int[size];
			} else if(!i_is_alloc){
				_buf = null;
			}
