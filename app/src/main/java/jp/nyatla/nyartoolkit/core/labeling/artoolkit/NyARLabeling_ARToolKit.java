...
/**
 * このクラスは、ARToolKitのアルゴリズムと互換性のあるラベリングクラスです。 入力画像に対して、インデクス化されたラべリング画像を出力します。
 */
final public class NyARLabeling_ARToolKit {
	...
	/**
	 * この関数は、ラスタをラべリングします。 結果は、o_destinationに出力します。
	 * <p>
	 * メモ - この関数の元になるARToolKitの関数は、static ARInt16 *labeling2( ARUint8 *image,
	 * int thresh,int *label_num, int **area, double **pos, int **clip,int
	 * **label_ref, int LorR )です。
	 * </p>
	 * 
	 * @param i_raster
	 *            入力元の二値ラスタオブジェクトです。画素形式は、{@link NyARBufferType#INT1D_BIN_8}
	 *            である必要があります。
	 * @param o_destination
	...
	public int labeling(NyARBinRaster i_raster, NyARLabelingImage o_destination)
		...
		for (j = 1; j < lysize - 1; j++) {// for (int j = 1; j < lysize - 1;j++,
											// pnt += poff*2, pnt2 += 2) {
			pixel_index = j * lxsize + 1;
			label_img_ptr1 = pixel_index - lxsize;// label_img_pt1 = label_img[j
													// - 1];
			for (i = 1; i < lxsize - 1; i++, pixel_index++, label_img_ptr1++) {// for(int
																				// i
																				// =
																				// 1;
																				// i
				...
				if (raster_buf[pixel_index] != 0) {
					label_img[pixel_index] = 0;// label_img_pt0[i] = 0;// *pnt2
												// = 0;
				} else {
					// pnt1 = ShortPointer.wrap(pnt2, -lxsize);//pnt1
					// =&(pnt2[-lxsize]);
					if (label_img[label_img_ptr1] > 0) {// if (label_img_pt1[i]
														// > 0) {// if( *pnt1 >
														// 0 ) {
						label_pixel = label_img[label_img_ptr1];// label_pixel =
																// label_img_pt1[i];//
																// *pnt2 =
																// *pnt1;
					...
					} else if (label_img[label_img_ptr1 + 1] > 0) {// } else if
																	// (label_img_pt1[i
																	// + 1] > 0)
																	// {// }else
																	// if(*(pnt1+1)
																	// > 0 ) {
						if (label_img[label_img_ptr1 - 1] > 0) {// if
																// (label_img_pt1[i
																// - 1] > 0) {//
																// if( *(pnt1-1)
																// > 0 ) {
							label_pixel = label_idxtbl[label_img[label_img_ptr1 + 1] - 1];// m
																							// =
																							// label_idxtbl[label_img_pt1[i
																							// +
																							// 1]
																							// -
																							// 1];//
																							// m
																							// =work[*(pnt1+1)-1];
							n = label_idxtbl[label_img[label_img_ptr1 - 1] - 1];// n
																				// =
																				// label_idxtbl[label_img_pt1[i
																				// -
																				// 1]
							...
							if (label_pixel > n) {
								...
								// &(work[0]);
								for (k = 0; k < wk_max; k++) {
									if (label_idxtbl[k] == label_pixel) {// if(
																			// *wk
																			// ==
																			// m
																			// )
							...
							work2_pt[2] += j;
							work2_pt[6] = j;
						} else if ((label_img[pixel_index - 1]) > 0) {// } else
																		// if
																		// ((label_img_pt0[i
																		// - 1])
																		// > 0)
							...
							label_pixel = label_idxtbl[label_img[label_img_ptr1 + 1] - 1];// m
																							...
																							// m
																							// =work[*(pnt1+1)-1];
							n = label_idxtbl[label_img[pixel_index - 1] - 1];// n
																				// =
																				// label_idxtbl[label_img_pt0[i
																				// -
																				// 1]
																				// -
																				// 1];//
							...
							label_pixel = label_img[label_img_ptr1 + 1];// label_pixel
																		// =
																		// label_img_pt1[i
																		// +
																		// 1];//
							...
							work2_pt[1] += i;// work2[((*pnt2)-1)*7+1] += i;
							work2_pt[2] += j;// work2[((*pnt2)-1)*7+2] += j;
							if (work2_pt[3] > i) {// if(work2[((*pnt2)-1)*7+3] >
													// i ){
								work2_pt[3] = i;// work2[((*pnt2)-1)*7+3] = i;
							}
							work2_pt[6] = j;// work2[((*pnt2)-1)*7+6] = j;
						}
					} else if ((label_img[label_img_ptr1 - 1]) > 0) {// } else
																		// if
																		// ((label_img_pt1[i
																		// - 1])
																		// > 0)
						...
						work2_pt[6] = j;// work2[((*pnt2)-1)*7+6] = j;
					} else if (label_img[pixel_index - 1] > 0) {// } else if
																// (label_img_pt0[i
																// - 1] > 0) {//
																// }else
																// if(*(pnt2-1)
																// > 0) {
						label_pixel = label_img[pixel_index - 1];// label_pixel
																	// =
																	// label_img_pt0[i
																	// - 1];//
																	// *pnt2
					...
					label_img[pixel_index] = label_pixel;// label_img_pt0[i] =
															// label_pixel;
				}
			}
		...
		for (i = 0; i < wk_max; i++) {// for(int i = 1; i <= wk_max; i++,wk++) {
			label_idxtbl[i] = (label_idxtbl[i] == i + 1) ? wlabel_num++
					: label_idxtbl[label_idxtbl[i] - 1];// *wk=(*wk==i)?j++:work[(*wk)-1];
		}
