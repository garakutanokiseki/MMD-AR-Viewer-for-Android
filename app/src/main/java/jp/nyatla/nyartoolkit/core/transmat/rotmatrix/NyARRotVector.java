...
/**
 * このクラスは、ベクトル(直線)から回転行列を計算する関数を定義します。 通常、ユーザがこのクラスを使うことはありません。
 * {@link NyARRotMatrix}クラスから使います。
 */
public class NyARRotVector {
	...
	/**
	 * コンストラクタです。 射影変換オブジェクトの参照値を設定して、インスタンスを作成します。
	 * 
	 * @param i_cmat
	...
	/**
	 * この関数は、２直線に直交するベクトルを計算して、その３次元ベクトルをインスタンスに格納します。 （多分）
	 * 
	 * @param i_linear1
	 *            直線１
	 * @param i_linear2
	 *            直線２
	 */
	public void exteriorProductFromLinear(NyARLinear i_linear1,
			NyARLinear i_linear2) {
		// 1行目
		...
		final double m0 = w1 * (cmat.m01 * cmat.m12 - cmat.m02 * cmat.m11) + w2
				* cmat.m11 - w3 * cmat.m01;// w1 * (cpara[0 * 4 + 1] * cpara[1 *
											// 4 + 2] - cpara[0 * 4 + 2] *
											// cpara[1 * 4 + 1]) + w2 * cpara[1
											// * 4 + 1] - w3 * cpara[0 * 4 + 1];
		final double m1 = -w1 * cmat.m00 * cmat.m12 + w3 * cmat.m00;// -w1 *
																	// cpara[0 *
																	// 4 + 0] *
																	// cpara[1 *
																	// 4 + 2] +
																	...
																	// cpara[0 *
																	// 4 + 0];
		final double m2 = w1 * cmat.m00 * cmat.m11;// w1 * cpara[0 * 4 + 0] *
													// cpara[1 * 4 + 1];
	 ...
	 * @throws NyARException
	 */
	public void checkVectorByVertex(final NyARDoublePoint2d i_start_vertex,
			final NyARDoublePoint2d i_end_vertex) throws NyARException {
		double h;
		NyARDoubleMatrix44 inv_cpara = this._inv_cpara;
		// final double[] world = __checkVectorByVertex_world;// [2][3];
		final double world0 = inv_cpara.m00 * i_start_vertex.x * 10.0
				+ inv_cpara.m01 * i_start_vertex.y * 10.0 + inv_cpara.m02
				* 10.0;// mat_a->m[0]*st[0]*10.0+
		final double world1 = inv_cpara.m10 * i_start_vertex.x * 10.0
				+ inv_cpara.m11 * i_start_vertex.y * 10.0 + inv_cpara.m12
				* 10.0;// mat_a->m[3]*st[0]*10.0+
		final double world2 = inv_cpara.m20 * i_start_vertex.x * 10.0
				+ inv_cpara.m21 * i_start_vertex.y * 10.0 + inv_cpara.m22
				* 10.0;// mat_a->m[6]*st[0]*10.0+
		...
		final double camera0 = (cmat.m00 * world0 + cmat.m01 * world1 + cmat.m02
				* world2)
				/ h;
		final double camera1 = (cmat.m10 * world0 + cmat.m11 * world1 + cmat.m12
				* world2)
				/ h;

		// h = cpara[2 * 4 + 0] * world3 + cpara[2 * 4 + 1] * world4 + cpara[2 *
		// 4 + 2] * world5;
		h = cmat.m20 * world3 + cmat.m21 * world4 + cmat.m22 * world5;
		if (h == 0.0) {
			throw new NyARException();
		}
		final double camera2 = (cmat.m00 * world3 + cmat.m01 * world4 + cmat.m02
				* world5)
				/ h;
		final double camera3 = (cmat.m10 * world3 + cmat.m11 * world4 + cmat.m12
				* world5)
				/ h;

		final double v = (i_end_vertex.x - i_start_vertex.x)
				* (camera2 - camera0) + (i_end_vertex.y - i_start_vertex.y)
				* (camera3 - camera1);
		if (v < 0) {
			this.v1 = -this.v1;
			this.v2 = -this.v2;
	...
	/**
	 * この関数は、ARToolKitのcheck_rotationに相当する計算をします。 詳細は不明です。(2つのベクトルの関係を調整？)
	 * 
	 * @throws NyARException
	 */
	public final static void checkRotation(NyARRotVector io_vec1,
			NyARRotVector io_vec2) throws NyARException {
		double w;
		int f;
