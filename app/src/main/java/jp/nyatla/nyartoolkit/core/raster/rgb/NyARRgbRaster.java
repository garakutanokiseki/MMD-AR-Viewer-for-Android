...
/**
 * このクラスは、指定形式のバッファを持つRGBラスタです。 外部参照バッファ、内部バッファの両方に対応します。
 * <p>
 * 対応しているバッファタイプ-
 * <ul>
 * {@link NyARBufferType#INT1D_X8R8G8B8_32}
...
public class NyARRgbRaster extends NyARRgbRaster_BasicClass {
	 ...
	 *            ラスタのサイズ
	 * @param i_raster_type
	 *            ラスタのバッファ形式。 {@link NyARBufferType}に定義された定数値を指定してください。
	...
	protected boolean initInstance(NyARIntSize i_size, int i_raster_type,
		...
		case NyARBufferType.INT1D_X8R8G8B8_32:
			...
			this._reader = new NyARRgbPixelReader_INT1D_X8R8G8B8_32(
					(int[]) this._buf, i_size);
			break;
		case NyARBufferType.BYTE1D_B8G8R8X8_32:
			this._buf = i_is_alloc ? new byte[i_size.w * i_size.h * 4] : null;
			this._reader = new NyARRgbPixelReader_BYTE1D_B8G8R8X8_32(
					(byte[]) this._buf, i_size);
			break;
		case NyARBufferType.BYTE1D_R8G8B8_24:
			this._buf = i_is_alloc ? new byte[i_size.w * i_size.h * 3] : null;
			this._reader = new NyARRgbPixelReader_BYTE1D_R8G8B8_24(
					(byte[]) this._buf, i_size);
			break;
		case NyARBufferType.BYTE1D_B8G8R8_24:
			this._buf = i_is_alloc ? new byte[i_size.w * i_size.h * 3] : null;
			this._reader = new NyARRgbPixelReader_BYTE1D_B8G8R8_24(
					(byte[]) this._buf, i_size);
			break;
		case NyARBufferType.BYTE1D_X8R8G8B8_32:
			this._buf = i_is_alloc ? new byte[i_size.w * i_size.h * 4] : null;
			this._reader = new NyARRgbPixelReader_BYTE1D_X8R8G8B8_32(
					(byte[]) this._buf, i_size);
			break;
		case NyARBufferType.WORD1D_R5G6B5_16LE:
			this._buf = i_is_alloc ? new short[i_size.w * i_size.h] : null;
			this._reader = new NyARRgbPixelReader_WORD1D_R5G6B5_16LE(
					(short[]) this._buf, i_size);
			break;
		default:
			return false;
		}
	 ...
	 * 
	 * @return インスタンスがバッファを所有すれば、trueです。
	 */
	...
	/**
	 * この関数は、ラスタに外部参照バッファをセットします。 外部参照バッファの時にだけ使えます。
	 */
