 ...
 * <p>
 * コールバック関数 このクラスはいくつかの自己コールバック関数を持つ抽象クラスです。継承クラスでコールバック関数を実装して使います。
 * <ul>
 ...
 * </p>
 * <p>
 * ラベル輪郭のエントリーポイント - このエントリポイントは、ラベルを構成する塊の輪郭を指す１点です。ここから８方位輪郭検出を実行すると、
 * ラベルの輪郭を一周することができます。
 * </p>
 * <p>
 * 入力できる画素形式 -
 * <p>
 * {@link NyARBinRaster}を入力する場合
 * <ul>
 * <li>{@link NyARBufferType#INT1D_BIN_8}
 * </ul>
 * </p>
 * <p>
 * {@link NyARGrayscaleRaster}を入力する場合
 * <ul>
...
public abstract class NyARLabeling_Rle {
	...
	public NyARLabeling_Rle(int i_width, int i_height) throws NyARException {
		...
		this._rlestack = new RleInfoStack(i_width * i_height * 2048
				/ (320 * 240) + 32);
	...
	/**
	 * 検出するラベルのエリア（画素数）範囲を設定します。 この範囲にあるラベルのみが、結果に返されます。 初期値は、
	 * {@link #AR_AREA_MAX},{@link #AR_AREA_MIN}です。
	 * 
	 * @param i_max
	 *            エリアの最大値を指定します。
	 * @param i_min
	 ...
	 * @param i_out
	 * @param i_th
	 *            BINラスタのときは0,GSラスタの時は閾値を指定する。 この関数は、閾値を暗点と認識します。 暗点<=th<明点
	 * @return
	 */
	private final int toRel(int[] i_bin_buf, int i_st, int i_len,
			RleElement[] i_out, int i_th) {
		int current = 0;
		int r = -1;
		// 行確定開始
		int x = i_st;
	 ...
	 * @throws NyARException
	 */
	private final boolean addFragment(RleElement i_rel_img, int i_nof,
			int i_row_index, RleInfoStack o_stack) throws NyARException {
		int l = i_rel_img.l;
	...
	/**
	 * この関数は、2値イメージの{@link NyARBinRaster}ラスタをラベリングします。 検出したラベルは、自己コールバック関数
	 * {@link #onLabelFound}で通知します。
	 * 
	 * @param i_bin_raster
	...
	/**
	 * この関数は、2値イメージの{@link NyARBinRaster}ラスタの指定範囲をラベリングします。 検出したラベルは、自己コールバック関数
	 * {@link #onLabelFound}で通知します。
	 * 
	 * @param i_bin_raster
	 *            入力画像。対応する形式は、クラスの説明を参照してください。
	 ...
	 * @throws NyARException
	 */
	public void labeling(NyARBinRaster i_bin_raster, NyARIntRect i_area)
			throws NyARException {
		assert (i_bin_raster.isEqualBufferType(NyARBufferType.INT1D_BIN_8));
		this.imple_labeling(i_bin_raster, 0, i_area.x, i_area.y, i_area.w,
				i_area.h);
	}
	 ...
	 * @throws NyARException
	 */
	public void labeling(NyARGrayscaleRaster i_gs_raster, int i_th)
			throws NyARException {
	 ...
	 * @throws NyARException
	 */
	public void labeling(NyARGrayscaleRaster i_gs_raster, NyARIntRect i_area,
			int i_th) throws NyARException {
		assert (i_gs_raster.isEqualBufferType(NyARBufferType.INT1D_GRAY_8));
		this.imple_labeling(i_gs_raster, i_th, i_area.x, i_area.y, i_area.w,
				i_area.h);
	}

	private void imple_labeling(INyARRaster i_raster, int i_th, int i_left,
			int i_top, int i_width, int i_height) throws NyARException {
		// ラスタのサイズを確認
		...
		for (int y = i_top + 1; y < bottom; y++) {
			...
			len_current = toRel(in_buf, rle_top_index, i_width, rle_current,
					i_th);
			int index_prev = 0;
	...
	/**
	 * この仮想関数は自己コールバック関数です。 {@link #labeling}関数が、検出したラベルを通知するために使います。
	 * 
	 * @param i_ref_label
	...
	/**
	 * クラスの仕様確認フラグです。ラベル配列の参照アクセスが可能かを返します。 このクラスでは、true固定です。
	 */
	public final static boolean _sf_label_array_safe_reference = true;
}

/**
 * このクラスは、{@link NyARLabeling_Rle}が内部的に使うRLEスタックです。 ユーザが使うことはありません。
 */
...
/**
 * このクラスは、{@link RleInfoStack}の要素です。 RLEフラグメントのパラメータを保持します。 ユーザが使うことはありません。
 */
class RleElement {
	int l;
	int r;
