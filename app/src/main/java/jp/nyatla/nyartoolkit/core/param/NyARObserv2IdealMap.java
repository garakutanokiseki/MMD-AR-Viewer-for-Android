...
/**
 * このクラスは、観察座標系を理想座標系へ変換するテーブルです。 座標系を変換するテーブルと、変換機能を提供します。
 */
public class NyARObserv2IdealMap {
	...
	/**
	 * コンストラクタです。 入力した{@link NyARCameraDistortionFactor}とそのサイズから、テーブルを作成します。
	 * ２つのパラメータは整合性が取れていなければなりません。 (通常は、{@link NyARParam}の
	 * {@link NyARParam#getDistortionFactor()},{@link NyARParam#getScreenSize()}
	 * から得られた パラメータを入力します。)
	 * 
	 * @param i_distfactor
	 *            樽型歪みパラメータのオブジェクト。
	 ...
	 *            スクリーンサイズ
	 */
	public NyARObserv2IdealMap(NyARCameraDistortionFactor i_distfactor,
			NyARIntSize i_screen_size) {
		NyARDoublePoint2d opoint = new NyARDoublePoint2d();
		int size = i_screen_size.w * i_screen_size.h;
		if(size != prevSize){
			_mapx = new double[size];
			_mapy = new double[size];
		}
		prevSize = size;
	...
	/**
	 * この関数は、観察座標を理想座標へ変換します。 入力できる値範囲は、コンストラクタに設定したスクリーンサイズの範囲内です。
	 * 
	 * @param ix
	 *            観察座標の値
	 * @param iy
	...
	/**
	 * この関数は、一括して観察座標を理想座標へ変換します。 入力できる値範囲は、コンストラクタに設定したスクリーンサイズの範囲内です。
	 * 
	 * @param i_coord
	 *            観察座標の配列
	 * @param i_start
	 *            変換対象にする配列の開始インデクス。
	 * @param i_num
	 *            変換対象にする要素の数。 i_startから、i_start+i_num-1までの要素を変換します。
	 * @param o_x_coord
	 *            変換したX座標を受け取る配列。
	 * @param o_y_coord
	 *            変換したY座標を受け取る配列。
	 ...
	 */
	public void observ2IdealBatch(NyARIntPoint2d[] i_coord, int i_start,
			int i_num, double[] o_x_coord, double[] o_y_coord,
			int i_out_start_index) {
		int idx;
