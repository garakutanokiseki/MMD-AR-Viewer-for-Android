...
public class NyARTransMatResult extends NyARDoubleMatrix44 {
	/**
	 * この行列に1度でも行列をセットしたかを返します。 {@link INyARTransMat#transMatContinue}関数が使います。
	 */
	public boolean has_value = false;
	/**
	 * 観測値とのずれを示すエラーレート値です。{@link INyARTransMat}が更新します。
	 * エラーレートの意味は、実装クラスごとに異なることに注意してください。 ユーザからは読出し専用です。 {@link #has_value}
	 * がtrueの時に使用可能です。
	 */
	public double last_error;

	/**
	 * コンストラクタです。 初期値を設定したインスタンスを作成します。
	 */
	...
	/**
	 * この関数は、平行移動量と回転行列をセットして、インスタンスのパラメータを更新します。 通常、ユーザが使うことはありません。
	 ...
	 *            設定する平行移動量
	 */
	public final void setValue(NyARDoubleMatrix33 i_rot,
			NyARDoublePoint3d i_trans, double i_error) {
		this.m00 = i_rot.m00;
		this.m01 = i_rot.m01;
		this.m02 = i_rot.m02;
		this.m03 = i_trans.x;
