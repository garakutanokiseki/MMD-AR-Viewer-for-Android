 ...
 * <p>
 * アルゴリズム - 2つの点集合の総当たりの距離マップを作り、その距離が小さいのものから順に抽出することで、
 * 其々の点の移動距離が最小になる組み合わせを計算します。
 * </p>
 * <p>
 * 使い方　- このクラスは、まず距離マップを作るために距離値をセットして、次に組み合わせを得る手順で使います。
 * 距離マップには行と列があります。列を基準値、行を比較値として、その距離値を格納します。 行と列の距離マップを作り終えたら、組合せを計算します。
 * <ol>
...
public class NyARDistMap {
	...
	/**
	 * コンストラクタです。 マップの最大サイズを指定して、インスタンスを作成します。
	 * 
	 * @param i_max_col
	 *            マップの最大列数
	 * @param i_max_row
	...
	/**
	 * この関数は、マップのサイズを指定します。 値は初期化されません。
	 * 
	 * @param i_col
	 *            新しい列数。
	 * @param i_row
	 ...
	 * <p>
	 * 注意 - このAPIは低速です。性能が必要な時は、{@link #setPointDists}を参考に、一括書込みする関数を検討してください。
	 * </p>
	 * 
	 * @param i_col
	 *            列のインデクスを指定します。
	 ...
	 *            i_vertex_cの有効な要素数
	 */
	public void setPointDists(NyARIntPoint2d[] i_vertex_r, int i_row_len,
			NyARIntPoint2d[] i_vertex_c, int i_col_len) {
	 ...
	 * 
	 * @param o_rowindex
	 *            組合せを受け取る配列です。 col[n]に対するrow[m]のインデクス番号mを、o_rowindex[n]に返します。
	 */
