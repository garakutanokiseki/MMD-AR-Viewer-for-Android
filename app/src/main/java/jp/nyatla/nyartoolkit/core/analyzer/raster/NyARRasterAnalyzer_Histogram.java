...
public class NyARRasterAnalyzer_Histogram {
	...
	/**
	 * コンストラクタです。 ラスタの画素形式を指定して、ヒストグラム分析器を作ります。
	 * 
	 * @param i_raster_format
	 *            入力するラスタの画素形式を指定します。 {@link NyARBufferType#INT1D_GRAY_8}
	 *            以外の画素形式の場合は、解析時にグレースケール化を行います。
	 * @param i_vertical_interval
	 *            画素スキャン時の、Y軸方向のスキップ数を指定します。数値が大きいほど高速に解析できますが、精度は劣化します。0
	 *            以上を指定してください。
	 * @throws NyARException
	 */
	public NyARRasterAnalyzer_Histogram(int i_raster_format,
			int i_vertical_interval) throws NyARException {
	...
	/**
	 * インスタンスを初期化します。 継承クラスを実装するときには、この関数をオーバライドしてください。
	 * 
	 * @see #NyARRasterAnalyzer_Histogram(int i_raster_format,int
	 *      i_vertical_interval)
	 * @param i_raster_format
	 *            see Also.
	 * @param i_vertical_interval
	 *            see Also.
	 * @return インスタンスの初期化に成功するとTrueを返します。
	 */
	...
	public void analyzeRaster(INyARRaster i_input, NyARHistogram o_histogram)
		...
		this._histImpl.createHistogram(i_input, 0, 0, size.w, size.h,
				o_histogram.data, this._vertical_skip);
		return;
	}
	 ...
	 * @throws NyARException
	 */
	public void analyzeRaster(INyARRaster i_input, NyARIntRect i_area,
			NyARHistogram o_histogram) throws NyARException {
		...
		this._histImpl.createHistogram(i_input, i_area.x, i_area.y, i_area.w,
				i_area.h, o_histogram.data, this._vertical_skip);
		return;
	}

	/**
	 * このインタフェイスは、ヒストグラム解析関数を定義します。 このインタフェイスを継承して、画素フォーマット毎のヒストグラム解析関数を実装します。
	 */
	protected interface ICreateHistogramImpl {
		 ...
		 *            行スキップ数
		 */
		public void createHistogram(INyARRaster i_raster, int i_l, int i_t,
				int i_w, int i_h, int[] o_histogram, int i_skip);
	}

	class NyARRasterThresholdAnalyzer_Histogram_INT1D_GRAY_8 implements
			ICreateHistogramImpl {
		public void createHistogram(INyARRaster i_raster, int i_l, int i_t,
				int i_w, int i_h, int[] o_histogram, int i_skip) {
	...
	class NyARRasterThresholdAnalyzer_Histogram_INT1D_X8R8G8B8_32 implements
			ICreateHistogramImpl {
		public void createHistogram(INyARRaster i_raster, int i_l, int i_t,
				int i_w, int i_h, int[] o_histogram, int i_skip) {
			assert (i_raster
			...
			for (int y = i_h - 1; y >= 0; y -= i_skip) {
				...
				for (x = pix_count - 1; x >= pix_mod_part; x--) {
					v = input[pt++];
					o_histogram[((v & 0xff) + (v & 0xff) + (v & 0xff)) / 3]++;
				}
				for (; x >= 0; x -= 8) {
					v = input[pt++];
					o_histogram[((v & 0xff) + (v & 0xff) + (v & 0xff)) / 3]++;
	...
	class NyARRasterThresholdAnalyzer_Histogram_BYTE1D_RGB_24 implements
			ICreateHistogramImpl {
		public void createHistogram(INyARRaster i_raster, int i_l, int i_t,
				int i_w, int i_h, int[] o_histogram, int i_skip) {
			...
			for (int y = i_h - 1; y >= 0; y -= i_skip) {
				int x;
				for (x = pix_count - 1; x >= pix_mod_part; x--) {
					o_histogram[((input[pt + 0] & 0xff)
							+ (input[pt + 1] & 0xff) + (input[pt + 2] & 0xff)) / 3]++;
					pt += 3;
				}
				for (; x >= 0; x -= 8) {
					o_histogram[((input[pt + 0] & 0xff)
	...
	class NyARRasterThresholdAnalyzer_Histogram_BYTE1D_B8G8R8X8_32 implements
			ICreateHistogramImpl {
		public void createHistogram(INyARRaster i_raster, int i_l, int i_t,
				int i_w, int i_h, int[] o_histogram, int i_skip) {
			assert (i_raster
	...
	class NyARRasterThresholdAnalyzer_Histogram_BYTE1D_X8R8G8B8_32 implements
			ICreateHistogramImpl {
		public void createHistogram(INyARRaster i_raster, int i_l, int i_t,
				int i_w, int i_h, int[] o_histogram, int i_skip) {
			assert (i_raster
			...
			for (int y = i_h - 1; y >= 0; y -= i_skip) {
				int x;
				for (x = pix_count - 1; x >= pix_mod_part; x--) {
					o_histogram[((input[pt + 1] & 0xff)
							+ (input[pt + 2] & 0xff) + (input[pt + 3] & 0xff)) / 3]++;
					pt += 4;
				}
				for (; x >= 0; x -= 8) {
					o_histogram[((input[pt + 1] & 0xff)
	...
	class NyARRasterThresholdAnalyzer_Histogram_WORD1D_R5G6B5_16LE implements
			ICreateHistogramImpl {
		public void createHistogram(INyARRaster i_raster, int i_l, int i_t,
				int i_w, int i_h, int[] o_histogram, int i_skip) {
			assert (i_raster
			...
			for (int y = i_h - 1; y >= 0; y -= i_skip) {
				...
				for (x = pix_count - 1; x >= pix_mod_part; x--) {
					v = (int) input[pt++];
					o_histogram[(((v & 0xf800) >> 8) + ((v & 0x07e0) >> 3) + ((v & 0x001f) << 3)) / 3]++;
				}
				for (; x >= 0; x -= 8) {
					v = (int) input[pt++];
					o_histogram[(((v & 0xf800) >> 8) + ((v & 0x07e0) >> 3) + ((v & 0x001f) << 3)) / 3]++;
	...
	public static void main(String[] args) {
		try {
			NyARRgbRaster raster = new NyARRgbRaster(100, 100,
			...
			rect.h = 10;
			rect.w = 10;
			NyARRasterAnalyzer_Histogram ha = new NyARRasterAnalyzer_Histogram(
					raster.getBufferType(), 1);
