...
public class NyARMatchPattDeviationBlackWhiteData {
	...
	/**
	 * この関数は、画素データを格納した配列を返します。 {@link NyARMatchPatt_BlackWhite#evaluate}
	 * 関数から使います。
	 */
	public int[] refData() {
		return this._data;
	...
	/**
	 * この関数は、ラスタから差分画像を生成して、格納します。 制限事項として、
	 * {@link NyARBufferType#INT1D_X8R8G8B8_32}形式のラスタのみ、入力できます。
	 * 
	 * @param i_raster
	...
	public void setRaster(INyARRaster i_raster) {
		...
		for (i = number_of_pixels - 1; i >= 0; i--) {
			rgb = buf[i];
			w_sum = ((255 * 3 - (rgb & 0xff) - ((rgb >> 8) & 0xff) - ((rgb >> 16) & 0xff)) / 3)
					- ave;
			linput[i] = w_sum;
			sum += w_sum * w_sum;
		}
