...
/**
 * このクラスは、色調フィルタのベースクラスです。 継承クラスで{@link #table}に変換ルールを書き込む処理を実装します。
 * <p>
 * 対応している画素形式は以下の通りです。
...
public class NyARRasterFilter_CustomToneTable implements INyARRasterFilter {
	...
	/**
	 * コンストラクタです。 入力/出力ラスタの画素形式を指定して、インスタンスを生成します。
	 * 
	 * @param i_raster_type
	 *            ラスタ形式。
	...
	/**
	 * 変換テーブルに従って、画素値を交換した画像を出力します。 画素形式は、コンストラクタに指定した形式に合せてください。
	 */
