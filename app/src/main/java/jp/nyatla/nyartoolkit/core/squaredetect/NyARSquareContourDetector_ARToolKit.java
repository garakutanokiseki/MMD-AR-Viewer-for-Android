...
/**
 * このクラスは、{@link NyARLabeling_ARToolKit}クラスを用いた矩形検出器です。 検出した矩形を、自己コールバック関数
 * {@link #onSquareDetect}へ通知します。 継承クラスで自己コールバック関数{@link #onSquareDetect}
 * を実装する必要があります。
 */
public abstract class NyARSquareContourDetector_ARToolKit extends
	...
	private final NyARLabelOverlapChecker<NyARLabelingLabel> _overlap_checker = new NyARLabelOverlapChecker<NyARLabelingLabel>(
			32, NyARLabelingLabel.class);
	...
	public final void detectMarker(NyARBinRaster i_raster) throws NyARException {
		...
		for (; i < label_num; i++) {
			...
			if (label_pt.clip_l == 1 || label_pt.clip_r == xsize - 2) {// if(wclip[i*4+0]
																		// == 1
																		// ||
																		// wclip[i*4+1]
																		// ==xsize-2){
			...
			if (label_pt.clip_t == 1 || label_pt.clip_b == ysize - 2) {// if(
																		// wclip[i*4+2]
																		// == 1
																		// ||
																		// wclip[i*4+3]
			...
			// 輪郭を取得
			if (!this._cpickup
					.getContour(limage, limage.getTopClipTangentX(label_pt),
