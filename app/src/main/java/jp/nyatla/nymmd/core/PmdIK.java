...
public class PmdIK {
	...
	public PmdIK(PMD_IK pPMDIKData, PmdBone[] i_ref_bone_array) {
		...
		if(0 < pPMDIKData.punLinkNo.length){
	...
	public void update() {
		...
		for (int it = 0; it < m_unCount; it++) {
			for (int cbLinkIdx = 0; cbLinkIdx < this.m_ppBoneList.length; cbLinkIdx++) {
				...
				//
				// 回転角
				double fRotAngle = Math.acos(vec3EffPos
						.Vector3DotProduct(vec3TargetPos));
				...
				if (0.00000001 < Math.abs(fRotAngle)) {
					...
					m_ppBoneList[cbLinkIdx].m_vec4Rotate.QuaternionMultiply(
							m_ppBoneList[cbLinkIdx].m_vec4Rotate, vec4RotQuat);
					m_ppBoneList[cbLinkIdx].m_vec4Rotate
							.QuaternionNormalize(m_ppBoneList[cbLinkIdx].m_vec4Rotate);
