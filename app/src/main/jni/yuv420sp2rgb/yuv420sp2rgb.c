...
JNIEXPORT void JNICALL Java_jp_androidgroup_nyartoolkit_ARToolkitDrawer_decodeYUV420SPMod(JNIEnv * env, jobject obj, jintArray rgb, jbyteArray yuv420sp, jint width, jint height, jint type)
{
    void *in, *out;
    int psz = getpagesize();
    int header_size;
    ...
    jboolean isCopyIn, isCopyOut;
    in = (*env)->GetDirectBufferAddress(env, yuv420sp);
    out = (*env)->GetDirectBufferAddress(env, rgb);
